...
| §19–20 | Gate regresi + shadow mode | **TERIMPLEMENTASI (konten/transport)** | `content-patch-gate.js` (`guarded-patch-v1`); mode `off/shadow/canary` di `content-canary.js`; `tests/cf-shadow-mode-test.js` (36 cek) — shadow transport, BUKAN shadow kebijakan Brain |
...
- **12 modul Brain murni** di `features/brain/` (4.939 baris total per hitungan Sonnet,
  ...
  Node per modul. Gate yang dieksekusi PASS oleh council: `tests/core-brain-v2-test.js`,
  `tests/core-brain-v3-upgrade-test.js`, `tests/tutor-brain-v3-test.js`, `tests/mastery-bkt-test.js`,
  `tests/regression-test.js` (Sol §1).
...
- **Mesin evolusi konten** end-to-end: `content-patch-gate.js` → `content-canary.js` (exposure
  ...
  level otonomi (`fiezel-autonomy-config.js`). `tests/content-canary-test.js` PASS.
...
- **Ritual rilis koheren**: SW_REV + DIAG_BUILD + FIEZEL_PAGE_BUILD naik bersama, ditegakkan
  `tests/install-health-test.js` dan `tests/pwa-release-coherence-test.js`.
...
## ADENDUM 2026-08-29 (wave 4)

Badan dokumen di atas SENGAJA tidak ditulis ulang — ia potret audit 2026-08-28 dan tetap sah
sebagai baseline. Adendum ini mencatat baris matriks §2 yang statusnya BERUBAH oleh wave
implementasi di branch yang sama, diverifikasi ulang file:baris + eksekusi gate oleh penulis
adendum pada 2026-08-29 (termasuk perubahan working tree yang belum di-commit — nomor baris
bisa bergeser saat parent merapikan commit).

| § Spec (baris lama) | Status lama | Status baru | Bukti baru |
|---|---|---|---|
| §7 antrean offline klien + retry | TIDAK ADA | **TERIMPLEMENTASI + WIRED, OFF** | 3 modul murni `features/telemetry/fiezel-learning-{events,queue,transport}.js` + saklar `fiezel-telemetry-config.js` (`enabled:false`, `mode:'off'`, `endpoint:''` — baris 25–28); dimuat `index.html:354–357`, precache `sw.js:79`; `node tests/learning-telemetry-test.js` → `LearningTelemetry: PASS` (dieksekusi ulang 2026-08-29). Pembaruan sinkronisasi 2026-08-29 (setelah wave lanjutan mendarat di branch yang sama): emitter di `app.js` kini ADA — `learningTelemetryEmitAnswer` (`app.js:1311`) dipanggil dari cabang grammar `record()` (`app.js:1395`), cabang mode `'off'` keluar paling dulu (`app.js:1314`) sehingga default tetap nol efek runtime; gate `tests/app-telemetry-wiring-test.js` → `AppTelemetryWiring: PASS` |
| §9 ingestion "tanpa idempotency" | PARSIAL (~85%) | **Dedup per-`batchId` TERIMPLEMENTASI (undeployed)** | `route-events.js:46` (`BATCH_ID_PATTERN`), `:156,:191,:295,:315` (dedup sinkron; replay → `200 duplicate:true`); tabel `batch_dedup` via `workers/api/migrations/0006_analytics_batch_dedup.sql` (+salinan byte-identik di `analytics/migrations/`); `node tests/analytics-dedup-test.js` → `AnalyticsDedup: PASS`. Catatan jujur (diperbarui saat sinkronisasi 2026-08-29): dedup per-`eventId` yang dituntut [BRAIN-TELEMETRY-SCHEMA.md](BRAIN-TELEMETRY-SCHEMA.md) §5 kini ADA — tetapi di lane learning (tabel `learning_dedup`, `workers/api/migrations/0007_learning.sql:57`; `INSERT OR IGNORE` per-event di `workers/api/learning/learning-store-d1.js:53`; gate `tests/learning-lane-test.js` → 44/44 PASS), bukan di lane analytics — lane analytics TETAP per-batch saja, by design |
| Lane ingest learning server (`/api/learning/events`) | (tidak dinilai audit lama) | **TERIMPLEMENTASI (undeployed, `LEARNING_ENABLED="off"`)** | Modul `workers/api/learning/` (`learning-core.js`, `route-learning-events.js`, `learning-store-d1.js`); rute terdaftar via `registerLearningRoutes` (`workers/api/route-wiring.js:74,549`); flag `LEARNING_ENABLED = "off"` (`workers/api/wrangler.toml:99`) + binding `LEARNING_DB` dengan `database_id` masih placeholder (`wrangler.toml:145–147`); migrasi `0007_learning.sql` (agregat `learning_daily` + dedup per-event `learning_dedup`) + entri `MIGRATIONS.md:19,83`; gate `tests/learning-lane-test.js` → 44/44 `LearningLane: PASS` (dieksekusi ulang 2026-08-29). Transport klien default menunjuk ke sana (`fiezel-learning-transport.js:221`) — kedua sisi kini ada di tree; yang tersisa deploy + aktivasi, urutannya di [BRAIN-ACTIVATION-RUNBOOK.md](BRAIN-ACTIVATION-RUNBOOK.md) |
| §17 simulator | TERIMPLEMENTASI (~80%), 3 profil, 1 seed, tanpa CI, bug censoring | **DIKERASKAN — dan verdict berubah PASS → FAIL (temuan jujur)** | `adaptivity-simulation-v3.js` kini 50 seed berpasangan × 9 profil (+3 profil bank), field `censored` per run, ambang praktis per metrik menggantikan `EPS=1e-9`, verdict via `FiezelStatGate.pairedBootstrap` CI 95%. `node adaptivity-simulation-v3.js` → **exit 1** (dieksekusi ulang 2026-08-29): gate kalibrasi FAIL — perbaikan itemBiasRMSE 0.2694→0.2622 di bawah ambang praktis 0.02, dan di 50 seed RMSE meanDiff −0.0028 CI [−0.0120, +0.0064] melintasi nol (inconclusive); censoring parah — varian bank tak pernah mastery pada 43/50 seed (rate 95,3%), v1→v2 rate 55,6%→89,8% CI selisih [0.298, 0.387]; klaim residual "menurunkan osilasi" tidak bertahan antar-seed (meanDiff +0.167, CI [+0.054, +0.275] — justru menambah osilasi). Yang tetap kuat untuk v2: accuracyGap, retention, brier. Gate mesin verdict sendiri PASS (`tests/adaptivity-simulation-v3-hardened-test.js`, 58 asersi). FAIL ini bukti untuk MASTER, bukan bug untuk "diperbaiki" dengan melonggarkan gate |
| §18 evaluasi kandidat "mentoleransi kalah 2/5, tanpa CI" | PARSIAL (~50%) | **TERIMPLEMENTASI (gate statistik)** | `features/brain/fiezel-stat-gate.js` (Wilson/Newcombe, bootstrap berpasangan, verdict non-inferioritas) → `StatGate: PASS`; `content-promotion.js` kini memakainya (ambang 8-attempt tinggal guard runtime-safety, bukan bukti promosi — menutup temuan §4.3) |
| §26–28 manifest versi | TERFRAGMENTASI | **TERIMPLEMENTASI + WIRED (deskriptif)** | `features/brain/fiezel-brain-manifest.js` memetakan 4 identitas versi + peta otoritas per modul; bersama `fiezel-brain-config.js`, `fiezel-learning-metrics.js`, `fiezel-metrics-digest.js` dimuat shell (`index.html:344–348`, `sw.js:78`); keempat gate PASS (dieksekusi ulang). Otoritas runtime tetap `off` — pembaruan sinkronisasi 2026-08-29: `FiezelBrainManifest` kini punya pemanggil di `app.js`, tetapi HANYA untuk tampilan/atribusi (panel diagnostik `brainManifestMarkup`, `app.js:7587`; `bundleVersion` opsional di ctx event, `app.js:1342`); keempat modul lain tetap tanpa pemanggil (grep = 0 hit) dan tidak ada yang ikut memutuskan apa pun |
| §12 metrik longitudinal (lokal) | TIDAK ADA (server) / PARSIAL (lokal) | **TERIMPLEMENTASI on-device (belum di-wire ke data nyata)** | `fiezel-learning-metrics.js` (gain, retensi, Brier, hint, persistensi miskonsepsi) + `fiezel-metrics-digest.js` (supresi k=20, enum tertutup) — sesuai relokasi keputusan #4; emitter kini ada (`app.js:1311,1395`), sehingga yang ditunggu bukan lagi emitter melainkan AKTIVASI aliran data (mode klien masih `'off'` + lane server belum deploy) |

Dua nilai konfigurasi yang dikutip badan dokumen juga sudah bergeser: `ANALYTICS_ENABLED` kini
`"on"` di `workers/api/wrangler.toml:92` (audit lama membacanya `"off"` di baris 65 — nilai DAN
posisi baris berubah), tetapi ketiga `database_id` D1 masih placeholder (kini
`wrangler.toml:112,129,147` — bergeser lagi dari `105,122` karena blok `LEARNING_DB` mendarat)
sehingga kesimpulan "0% operasional / belum pernah deploy" **tetap berlaku**. Vonis §1 selebihnya
tidak berubah: yang bergeser adalah "emitter 0%" menjadi — per sinkronisasi 2026-08-29 —
"emitter TER-WIRE di `app.js` dan lane server TERIMPLEMENTASI, keduanya mati by default
(mode klien `'off'`, `LEARNING_ENABLED="off"`, worker belum deploy) menunggu aktivasi bertahap",
dan itu persis urutan yang diatur [BRAIN-ACTIVATION-RUNBOOK.md](BRAIN-ACTIVATION-RUNBOOK.md).
