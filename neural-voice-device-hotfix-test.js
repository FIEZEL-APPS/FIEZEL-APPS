...
for(const phase of ['bootstrap_lifecycle_watch_ready','bootstrap_lifecycle_visibilitychange','bootstrap_lifecycle_pagehide','bootstrap_lifecycle_pageshow','bootstrap_lifecycle_beforeunload'])assert.ok(boot.includes(`phase:'${phase}'`),`m025-12 missing bootstrap lifecycle diagnostic ${phase}`);
...
assert.ok(bootstrapLifecycleStart>=0&&bootstrapLifecycleEnd>bootstrapLifecycleStart,'m025-12 bootstrap lifecycle watcher must install at runtime load');
...
assert.ok(sw.includes("const SW_REV='m025-12-startup-readiness-"),'m025-12 startup-readiness deploy must force a matching shell refresh while keeping the m025-5 vendor/cache namespace');
assert.ok(diag.includes("var DIAG_BUILD = 'm025-12';"),'diagnostics build must advance exactly to m025-12 for startup-readiness deploy');
...
assert.ok(watchdogStart>=0,'m025-12 must retain the bounded event-loop watchdog phase');
...
assert.ok(coreSrc.includes("policy: appleStandalone ? 'apple-standalone-inference-slice-v2' : 'default'"),'m025-12 must retain the effective Apple inference-slice policy');
assert.ok(coreSrc.includes('const appleHardChunkChars = appleStandalone ? Math.max(64, Math.min(128, Number(options.appleHardChunkChars) || 80)) : 0;'),'m025-12 Apple hard chunk cap must remain at 80 chars and narrowly bounded');
assert.ok(coreSrc.includes("phase: 'chunk_plan'")&&coreSrc.includes('maxChunkChars'),'m025-12 must expose chunk-count/max-char metadata without text content');
assert.ok(coreSrc.includes("phase: 'generate_completed_over_budget'"),'m025-12 must retain synchronous diagnosis for non-preemptive generation past budget');
assert.ok(coreSrc.includes("phase: 'prefetch_policy_ready'")&&coreSrc.includes("apple-standalone-macrotask-yield-v1"),'m025-12 must retain the Apple prefetch-yield policy');
assert.ok(coreSrc.includes("phase: 'prefetch_event_loop_yield'")&&coreSrc.includes('await new Promise(resolve => setTimeout(resolve, 0))'),'m025-12 must retain one macrotask yield before Apple next-chunk prefetch');
...
for(const phase of ['lifecycle_watch_ready','lifecycle_visibilitychange','lifecycle_pagehide','lifecycle_pageshow','lifecycle_beforeunload'])assert.ok(coreSrc.includes(`phase: '${phase}'`),`m025-12 must retain core lifecycle diagnostic ${phase}`);
...
assert.ok(probeRefresh>=0&&probeEnsure>probeRefresh&&probeSpeak>probeEnsure,'m025-12 probe must refresh prepared marker, ensure readiness, then speak in order');
assert.ok(!probeSrc.includes('status&&status.prepared===true'),'probe must not gate readiness on a stale pre-refresh status snapshot');
...
console.log('FIEZEL neural device hotfix m025-5 vendor / m025-12 startup-readiness deploy: PASS');
