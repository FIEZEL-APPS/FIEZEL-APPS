...
assert.ok(sw.includes("const SW_REV='m025-10-apple-inference-slice-"),'m025-10 Apple inference-slice deploy must force a matching shell refresh while keeping the m025-5 vendor');
assert.ok(diag.includes("var DIAG_BUILD = 'm025-10';"),'diagnostics build must advance exactly to m025-10 for the Apple inference-slice deploy');
...
assert.ok(watchdogStart>=0,'m025-10 core must retain the bounded event-loop watchdog phase');
...
assert.ok(coreSrc.includes("policy: appleStandalone ? 'apple-standalone-inference-slice-v2' : 'default'"),'m025-10 must expose the effective Apple inference-slice policy');
assert.ok(coreSrc.includes('const appleHardChunkChars = appleStandalone ? Math.max(64, Math.min(128, Number(options.appleHardChunkChars) || 80)) : 0;'),'m025-10 Apple hard chunk cap must default to 80 chars and remain narrowly bounded');
assert.ok(coreSrc.includes("phase: 'chunk_plan'")&&coreSrc.includes('maxChunkChars'),'m025-10 must expose chunk-count/max-char metadata without text content');
assert.ok(coreSrc.includes("phase: 'generate_completed_over_budget'"),'m025-10 must retain synchronous diagnosis for non-preemptive generation past budget');
...
assert.ok(appleProbeChunks.length>=8,'Apple physical probe must be decomposed into materially smaller inference slices');
...
console.log('FIEZEL neural device hotfix m025-5 vendor / m025-10 Apple inference-slice deploy: PASS');