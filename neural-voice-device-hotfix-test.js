...
assert.ok(sw.includes("const SW_REV='m025-9-apple-char-cap-"),'m025-9 Apple chunk-cap deploy must force a matching shell refresh while keeping the m025-5 vendor');
assert.ok(diag.includes("var DIAG_BUILD = 'm025-9';"),'diagnostics build must advance exactly to m025-9 for the Apple chunk-cap deploy');
...
assert.ok(watchdogStart>=0,'m025-9 core must retain the bounded event-loop watchdog phase');
...
assert.ok(coreSrc.includes("policy: appleStandalone ? 'apple-standalone-char-cap-v1' : 'default'"),'m025-9 must expose the effective Apple chunk policy');
assert.ok(coreSrc.includes('const appleHardChunkChars = appleStandalone ? Math.max(160, Math.min(320, Number(options.appleHardChunkChars) || 240)) : 0;'),'m025-9 Apple hard chunk cap must default to 240 chars and remain bounded');
assert.ok(coreSrc.includes("phase: 'chunk_plan'")&&coreSrc.includes('maxChunkChars'),'m025-9 must expose chunk-count/max-char metadata without text content');
assert.ok(coreSrc.includes("phase: 'generate_completed_over_budget'"),'m025-9 must synchronously diagnose non-preemptive generation completion over the configured budget');
...
console.log('FIEZEL neural device hotfix m025-5 vendor / m025-9 Apple chunk-cap deploy: PASS');