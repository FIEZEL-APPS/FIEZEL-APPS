...
assert.ok(sw.includes("const SW_REV='m025-11-apple-event-loop-yield-"),'m025-11 Apple event-loop-yield deploy must force a matching shell refresh while keeping the m025-5 vendor');
assert.ok(diag.includes("var DIAG_BUILD = 'm025-11';"),'diagnostics build must advance exactly to m025-11 for the Apple event-loop-yield deploy');
...
assert.ok(watchdogStart>=0,'m025-11 core must retain the bounded event-loop watchdog phase');
...
assert.ok(coreSrc.includes("policy: appleStandalone ? 'apple-standalone-inference-slice-v2' : 'default'"),'m025-11 must retain the effective Apple inference-slice policy');
assert.ok(coreSrc.includes('const appleHardChunkChars = appleStandalone ? Math.max(64, Math.min(128, Number(options.appleHardChunkChars) || 80)) : 0;'),'m025-11 Apple hard chunk cap must remain at 80 chars and narrowly bounded');
assert.ok(coreSrc.includes("phase: 'chunk_plan'")&&coreSrc.includes('maxChunkChars'),'m025-11 must expose chunk-count/max-char metadata without text content');
assert.ok(coreSrc.includes("phase: 'generate_completed_over_budget'"),'m025-11 must retain synchronous diagnosis for non-preemptive generation past budget');
assert.ok(coreSrc.includes("phase: 'prefetch_policy_ready'")&&coreSrc.includes("apple-standalone-macrotask-yield-v1"),'m025-11 must expose the Apple prefetch-yield policy');
assert.ok(coreSrc.includes("phase: 'prefetch_event_loop_yield'")&&coreSrc.includes('await new Promise(resolve => setTimeout(resolve, 0))'),'m025-11 must yield one macrotask before Apple next-chunk prefetch');
...
for(const phase of ['lifecycle_watch_ready','lifecycle_visibilitychange','lifecycle_pagehide','lifecycle_pageshow','lifecycle_beforeunload'])assert.ok(coreSrc.includes(`phase: '${phase}'`),`m025-11 missing lifecycle diagnostic ${phase}`);
...
console.log('FIEZEL neural device hotfix m025-5 vendor / m025-11 Apple event-loop-yield deploy: PASS');
