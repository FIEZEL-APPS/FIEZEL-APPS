...
assert.ok(sw.includes("const SW_REV='m025-8-eventloop-watchdog-"),'m025-8 diagnostic watchdog deploy must force a matching shell refresh while keeping the m025-5 vendor');
assert.ok(diag.includes("var DIAG_BUILD = 'm025-8';"),'diagnostics build must advance exactly to m025-8 for the event-loop watchdog deploy');
...
assert.ok(watchdogStart>=0,'m025-8 core must emit the bounded event-loop watchdog phase');
const watchdogEnd=coreSrc.indexOf('}, eventLoopWatchdogMs);',watchdogStart);
assert.ok(watchdogEnd>watchdogStart,'m025-8 watchdog block must be bounded and scheduled by eventLoopWatchdogMs');
...
assert.equal(probeChunks.length,2,'160-word diagnostic passage must deterministically produce two natural chunks');
assert.deepEqual(probeChunks.map(x=>x.trim().split(/\s+/).length),[76,84],'device probe chunk boundaries must remain stable and natural');
...
console.log('FIEZEL neural device hotfix m025-5 vendor / m025-8 event-loop watchdog deploy: PASS');