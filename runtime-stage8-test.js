...
// Keep the review fixture keyed to the canonical lesson ID used by the current
// grammar curriculum. The history key above intentionally stays generic because
...
setTimeout(()=>{
  ...
  const checks={
    learningMap:(html.includes('Peta Belajar')||html.includes('Peta kemampuan')||html.includes('Kemajuan materi')),timeline:html.includes('Linimasa Kelemahan')&&html.includes(new Date(now).toISOString().slice(0,10)),smartReview:(html.includes('Ulangan Pintar')||html.includes('Waktunya Mengulang'))&&(html.includes('risiko lupa')||html.includes('bisa lupa')),confidence:html.includes('Kalibrasi Keyakinan'),errorPatterns:html.includes('Pola Kesalahan')&&html.includes('Subject verb agreement'),confusionNetwork:html.includes('Jaringan Kekeliruan Kosakata'),readingMap:html.includes('Peta Skill Reading'),diagnostic:true,creator:html.includes('instagram.svg')&&html.includes('@fitrarustqi'),
