...
(`index.html`, `features/ui/fiezel-zoom-lock.js`, `app-interaction-policy-test.js`), dan dua
...
- Urutan dua gelombang: seluruh aset dulu, **`sw.js` paling akhir**. Membaliknya = murid
  memegang shell campur. `deploy-site-gate-test.js` menegakkan urutan ini di sumber.
...
`app-interaction-policy-test.js` menguncinya supaya tetap terlihat. Panel itu permukaan
