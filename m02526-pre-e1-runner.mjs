...
const URL = 'http://127.0.0.1:8001/m02526-probe.html';

const ctxDir = '/home/user/workspace/pre-e1-profile';
...
async function visit(tag) {
  ...
  fs.writeFileSync(`/tmp/pre-e1-${tag}.json`, JSON.stringify(result, null, 2));
  fs.writeFileSync(`/tmp/pre-e1-${tag}.log`, logs.join('\n'));
  console.log(`=== ${tag} ===`);
