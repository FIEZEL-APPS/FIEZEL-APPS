...
function grammarExercise(skill,item,variant){const meta=grammarMeta(item),correct=meta.options[meta.correctIndex],reasons=Array.isArray(item?.[4])?item[4]:meta.options.map(()=>''),wrong=meta.options.map((option,index)=>({option,index,reason:reasons[index]||'',detail:(Array.isArray(item?.[17])?item[17]:[]).find(x=>String(x.option)===String(option))||{}})).filter(x=>x.index!==meta.correctIndex),mode=GRAMMAR_PRACTICE_MODES[variant]||GRAMMAR_PRACTICE_MODES[0],title=friendlySkillName(skill),base=`${meta.stem}`;
  ...
  if(variant===6)return metaChoice(`Kesalahan mikir apa yang memang mau dicegah sama lesson ${title}?`,'misconception','Nah, ini kesalahan mikir utama yang dibidik lesson ini — bukan sekadar salah eja.');
...
function grammarBorrowedOptionReason(option,sourceId){
  ...
  return skill
    ? `${quoteEmbed(option)} sebenarnya bener, tapi itu punya lesson ${friendlySkillName(skill)} - bukan pola yang lagi diuji di sini.`
...
function makeGrammarQuestion(skill,item,variant=0,lessonSkill=skill){const exercise=grammarExercise(skill,item,variant),marked=shuffle(exercise.options.map((option,index)=>({x:String(option),ok:index===exercise.answerIndex,reason:String(exercise.optionReasons?.[index]||''),expl:String(exercise.optionExplanations?.[index]||''),src:grammarNormalizeOptionSource(exercise.optionSources?.[index])}))),level=LEVELS.includes(item?.[5])?item[5]:'A1',focus=friendlySkillName(skill).toLowerCase(),rule=grammarRuleIndonesian(item),misMap=grammarMisconceptionKeys(item),lessonId=String(item?.[8]||''),
  ...
  distractors=marked.map(x=>({option:x.x,reason:x.ok?`${quoteEmbed(x.x)} bener, karena pas dengan fokus ${focus}.`:(x.expl||(x.src.origin!=='own'?grammarBorrowedOptionReason(x.x,x.src.sourceId):grammarOptionReason(x.x,false,x.reason,misMap[x.x]))),sourceId:x.src.origin==='own'?lessonId:x.src.sourceId,own:x.src.origin==='own'}));return{id:`grammar-${item?.[8]||skill}-${exercise.mode}-${Date.now()}-${Math.random()}`,type:'grammar',level,skill,lessonSkill,sourceId:item?.[8]||'',conceptId:item?.[8]||'',practiceMode:exercise.mode,canary:item?.[14]||null,question:exercise.question,options:marked.map(x=>x.x),answerIndex:marked.findIndex(x=>x.ok),difficulty:LEVELS.indexOf(level)+1,explain:{why:exercise.correctWhy,rule:`${rule} Fokus khusus: ${focus}.`,avoid:'Pahami dulu maksud kalimatnya, baru ikuti petunjuk dari lesson sebelum milih bentuk.',memory:`Inget fokus ${focus}, ya. Bedain kenapa jawaban benar itu beda dari tiap pilihan jebakannya.`,distractors,distractor:'Tiap pilihan salah bawa jebakan mikirnya sendiri. Cek alasannya satu-satu, jangan cuma pilih yang keliatan familier.'},
