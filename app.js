...
function grammarAlternativeMeta(item,field,count=3){const own=grammarMeta(item),pool=GRAMMAR_ITEMS.filter(x=>x.item!==item).map(x=>grammarMeta(x.item)?.[field]).filter(Boolean);const start=pool.length?stableGrammarHash(`${own.id}:${field}`)%pool.length:0,out=[];for(let i=0;i<pool.length&&out.length<count;i++){const value=String(pool[(start+i)%pool.length]||'').trim();if(value&&norm(value)!==norm(own[field])&&!out.some(x=>norm(x)===norm(value)))out.push(value)}const fallback=[tr('grammar.alt.1'),tr('grammar.alt.2'),tr('grammar.alt.3')];for(const value of fallback)if(out.length<count&&!out.some(x=>norm(x)===norm(value)))out.push(value);return out.slice(0,count)}
...
function grammarExercise(skill,item,variant){const meta=grammarMeta(item),correct=meta.options[meta.correctIndex],reasons=Array.isArray(item?.[4])?item[4]:meta.options.map(()=>''),wrong=meta.options.map((option,index)=>({option,index,reason:reasons[index]||'',detail:(item?.[12]?.distractors||[]).find(x=>String(x.option)===String(option))||{}})).filter(x=>x.index!==meta.correctIndex),mode=GRAMMAR_PRACTICE_MODES[variant]||GRAMMAR_PRACTICE_MODES[0],title=friendlySkillName(skill),base=`${meta.stem}`;
  ...
  const lower=title.toLowerCase();if(variant===0)return direct(base,meta.options,meta.correctIndex,tr('grammar.ex.v0.why',{correct,title:lower}),reasons);
  ...
  if(variant===3)return metaChoice(tr('grammar.ex.v3.q',{base}),'rule',tr('grammar.ex.v3.why'));
  if(variant===4)return metaChoice(tr('grammar.ex.v4.q',{base}),'objective',tr('grammar.ex.v4.why'));
  if(variant===5)return metaChoice(tr('grammar.ex.v5.q',{base}),'reasoning',tr('grammar.ex.v5.why'));
  if(variant===6)return metaChoice(tr('grammar.ex.v6.q',{title}),'misconception',tr('grammar.ex.v6.why'));
  if(variant===7)return metaChoice(tr('grammar.ex.v7.q',{base}),'memory',tr('grammar.ex.v7.why'));
  if(variant===8)return metaChoice(tr('grammar.ex.v8.q'),'avoid',tr('grammar.ex.v8.why'));
  if(variant>=9&&variant<=11){const target=wrong[variant-9];return direct(tr('grammar.ex.v9.q',{option:target.option,base}),[target.reason,meta.whyCorrect,...wrong.filter(x=>x!==target).map(x=>x.reason)],0,grammarOptionReason(target.option,false,target.reason));}
  if(variant>=12&&variant<=14){const target=wrong[variant-12],labels=wrong.filter(x=>x!==target).map(x=>String(x.detail.misconception||x.reason));return direct(tr('grammar.ex.v12.q',{option:target.option,base}),[String(target.detail.misconception||target.reason),tr('grammar.ex.v12.none'),...labels],0,tr('grammar.ex.v12.why',{option:target.option}));}
  if(variant>=15&&variant<=17){const target=wrong[variant-15];return direct(tr('grammar.ex.v15.q',{option:target.option,base}),meta.options,meta.correctIndex,tr('grammar.ex.v15.why',{correct}),reasons);}
  if(variant>=18&&variant<=20){const target=wrong[variant-18],failure=grammarOptionReason(target.option,false,target.reason);return direct(tr('grammar.ex.v18.q',{correct,option:target.option,base}),[tr('grammar.ex.v18.o1',{correct,failure}),tr('grammar.ex.v18.o2',{option:target.option,correct}),tr('grammar.ex.v18.o3'),tr('grammar.ex.v18.o4')],0,tr('grammar.ex.v18.why',{option:target.option}));}
  ...
  if(variant===22)return metaChoice(tr('grammar.ex.v22.q',{base}),'reasoning',tr('grammar.ex.v22.why'));
  if(variant===23){const correctSummary=`${meta.objective} ${meta.rule}`.trim(),alternatives=grammarAlternativeMeta(item,'objective',3).map((x,i)=>`${x} ${grammarAlternativeMeta(item,'rule',3)[i]}`.trim());return direct(tr('grammar.ex.v23.q',{title}),[correctSummary,...alternatives],0,tr('grammar.ex.v23.why'));}
  const correctPlan=`${meta.avoid} ${meta.memory}`.trim(),avoid=grammarAlternativeMeta(item,'avoid',3),memory=grammarAlternativeMeta(item,'memory',3);return direct(tr('grammar.ex.v24.q',{title}),[correctPlan,...avoid.map((x,i)=>`${x} ${memory[i]}`.trim())],0,tr('grammar.ex.v24.why'));
}
function indonesianPartOfSpeech(value){const raw=String(value||'').toLowerCase();if(!raw)return tr('vocab.pos.default');const key=`vocab.pos.${raw}`;return I18N.has(key)?tr(key):raw}
function readingStems(type){const list=I18N.list(`reading.stem.${type}`);return list.length?list:I18N.list('reading.stem.detail')}
function readingFocusLabel(type){const key=`reading.focus.${type}`;return I18N.has(key)?tr(key):tr('reading.focusDefault')}
...
function dailyBrief(){const due=dueItems();const profile=getDiagnosticProfile();const weak=Object.entries(profile.weakSkills).sort((a,b)=>b[1].score-a[1].score)[0]?.[0];return {review:due.length,weak:weak||tr('coach.noPattern'),goal:tr(state.adaptiveReady?'coach.goal.adaptive':'coach.goal.diagnostic')} }
...
function localCoachSignal(){const p=buildAdaptivePolicy();if(p.mode==='diagnostic')return{title:p.title,text:p.summary,action:p.cta,ready:false};const focus=p.targetSkill?friendlySkillName(p.targetSkill):friendlySkillName(p.primaryDomain);return{title:p.title,text:tr('coach.focus',{summary:p.summary,focus,n:p.sessionSize,min:p.estimatedMinutes}),action:p.cta,ready:true,policy:p}}
...
function todayLabel(){try{return new Intl.DateTimeFormat(I18N.intlLocale(),{weekday:'long',day:'numeric',month:'long'}).format(new Date())}catch{return dayKey(Date.now())}}
...
function reminderMessages(kind){const pool=I18N.list(`reminder.messages.${kind}`);return pool.length?pool:I18N.list('reminder.messages.starter')}
...
async function showStudyNotification(kind,body){
  if(notificationPermission()!=='granted'||!body)return false;const titles=reminderTitles(kind);const title=titles[Math.abs(Number(state.reminderMeta?.lastMessageIndex||0))%titles.length];const options={body,tag:`fiezel-study-${kind}`,renotify:false,icon:'./apple-touch-icon.png',badge:'./favicon-64.png',data:{url:'./'}};
...
function toggleSoundtrack(){state.preferences.soundtrack=!state.preferences.soundtrack;save();if(state.preferences.soundtrack){const ok=startSoundtrack();showToast(tr(ok?'toast.musicOn':'toast.musicBlocked'))}else{stopSoundtrack();showToast(tr('toast.musicOff'))}haptic('confirm');enhanceUI()}
...
function go(v){if(!VALID_VIEWS.has(v)){showToast(tr('toast.viewUnavailable'));return false}const swap=()=>{state.view=v;save();render()};if(document.startViewTransition&&state.preferences?.motion!==false)document.startViewTransition(swap);else swap();return true} window.go=go;
...
<div class="launcher-shell">
  <div class="launcher-copy">
    <div class="launcher-meta"><span class="live-signal"></span><span>${esc(tr('home.brandMeta'))} · ${esc(todayLabel())}</span>${celestialStatusMarkup()}</div>
    ...
    <p class="learner-stage"><i data-lucide="graduation-cap"></i> ${esc(tr('home.stage',{grade:tr('home.gradeLabel'),semester:LEARNER_STAGE.semester,year:LEARNER_STAGE.schoolYear}))}</p>
    <div class="launcher-actions"><button class="primary luxe" onclick="${primaryAction}">${state.adaptiveReady?esc(policy.cta):esc(tr('home.buildProfile'))} <i data-lucide="arrow-up-right"></i></button><button class="ghost-dark" onclick="askCoachAI()" data-testid="home-ai-coach-button"><i data-lucide="sparkles"></i> ${esc(tr('home.analyzeAI'))}</button></div>
  </div>
  <aside class="coach-preview">
    <div class="coach-head"><span class="coach-icon"><i data-lucide="brain-circuit"></i></span><div><small>${esc(tr('home.coach.kicker'))}</small><b>${esc(tr(state.adaptiveReady?'home.coach.active':'home.coach.collecting'))}</b></div></div>
    <div class="coach-level"><span>${state.adaptiveReady?esc(tr('home.coach.corePlan',{mode:policy.mode.toUpperCase()})):esc(tr('home.coach.estimatedLevel'))}</span><strong>${state.adaptiveReady?esc(tr('home.coach.questions',{n:policy.sessionSize})):esc(level)}</strong></div>
    <h2>${esc(signal.title)}</h2>
    <p>${esc(coachText)}</p>
    <button class="coach-link" onclick="askCoachAI()">${esc(tr('home.coach.open'))} <i data-lucide="arrow-right"></i></button>
  </aside>
</div>
...
<div class="learning-launcher">
  <button class="launch-card vocab-launch" onclick="go('vocab')"><span class="launch-icon"><i data-lucide="book-a"></i></span><span><small>${esc(tr('home.launch.words',{n:V.length.toLocaleString()}))}</small><b>${esc(tr('home.launch.vocab'))}</b></span><i data-lucide="arrow-up-right"></i></button>
  <button class="launch-card grammar-launch" onclick="go('grammar')"><span class="launch-icon"><i data-lucide="braces"></i></span><span><small>${esc(tr('home.launch.skills',{n:Object.keys(G).length}))}</small><b>${esc(tr('home.launch.grammar'))}</b></span><i data-lucide="arrow-up-right"></i></button>
  <button class="launch-card reading-launch" onclick="go('reading')"><span class="launch-icon"><i data-lucide="book-open-text"></i></span><span><small>${esc(tr('home.launch.passages',{n:R.length}))}</small><b>${esc(tr('home.launch.reading'))}</b></span><i data-lucide="arrow-up-right"></i></button>
  ${indonesianContentLaunchers()}
  <button class="launch-card skills-launch" onclick="go('skills')"><span class="launch-icon"><i data-lucide="audio-waveform"></i></span><span><small>${esc(tr('home.launch.skillsLab.sub'))}</small><b>${esc(tr('home.launch.skillsLab'))}</b></span><i data-lucide="arrow-up-right"></i></button>
</div>
</section>`)}
function indonesianContentLaunchers(){if(isThai())return '';return `<button class="launch-card library-launch" onclick="go('library')"><span class="launch-icon"><i data-lucide="library-big"></i></span><span><small>${esc(tr('home.launch.library.sub'))}</small><b>${esc(tr('home.launch.library'))}</b></span><i data-lucide="arrow-up-right"></i></button><button class="launch-card classroom-launch" onclick="go('classroom')"><span class="launch-icon"><i data-lucide="presentation"></i></span><span><small>${esc(tr('home.launch.classroom.sub'))}</small><b>${esc(tr('home.launch.classroom'))}</b></span><i data-lucide="arrow-up-right"></i></button>`}
...
function setNeuralVoicePreference(value){const next=String(value||'auto');state.preferences={...state.preferences,neuralVoice:next==='auto'||neuralVoiceCatalog().some(item=>item.id===next)?next:'auto'};save();const label=neuralVoiceCatalog().find(item=>item.id===state.preferences.neuralVoice)?.label||tr('voice.autoShort');showToast(tr('voice.toast.selected',{label}))}
...
function indonesianVoiceLabel(){const rt=self.FiezelIndonesianVoice;if(!rt)return tr('voice.id.missing');const st=rt.status();const mb=Math.round(Number(st.totalBytes||0)/1000000);return st.prepared?tr('voice.id.ready'):tr('voice.id.bundled',{mb})}
...
function neuralVoiceStatusMarkup(){const runtime=self.FiezelVoiceRuntime,status=runtime?.status?.()||{prepared:false,ready:false,phase:'unavailable',totalBytes:0};const size=Math.round(Number(status.totalBytes||0)/1000000)||119,label=tr(status.circuitOpen?'voice.status.circuit':status.ready?'voice.status.ready':status.prepared?'voice.status.prepared':'voice.status.none'),selected=selectedNeuralVoice(),catalog=neuralVoiceCatalog();const options=[`<option value="auto" ${selected==='auto'?'selected':''}>${esc(tr('voice.auto'))}</option>`,...catalog.map(item=>`<option value="${esc(item.id)}" ${selected===item.id?'selected':''}>${esc(item.label)} · ${esc(item.locale)}</option>`)].join('');return `<article class="voice-setup-card"><div class="voice-setup-copy"><h2>${esc(label)}</h2><p id="neuralVoiceProgress">${esc(status.prepared?tr('voice.stored'):tr('voice.download',{size}))}</p>${status.error?`<p class="muted">${esc(tr('voice.statusLine',{error:status.error,isolated:tr(status.crossOriginIsolated?'voice.yes':'voice.no'),tts:tr(status.speechSynthesis?'voice.available':'voice.unavailable')}))}</p>`:''}<label class="neural-voice-choice" for="neuralVoiceSelect"><span>${esc(tr('voice.model'))}</span><select id="neuralVoiceSelect" ${runtime?'':'disabled'}>${options}</select><small>${esc(tr('voice.modelHint'))}</small></label><label class="neural-voice-choice" for="neuralRateInput"><span>${esc(tr('voice.rate'))}</span><input id="neuralRateInput" type="range" min="${NEURAL_RATE_MIN}" max="${NEURAL_RATE_MAX}" step="${NEURAL_RATE_STEP}" value="${selectedNeuralRate()}" ${runtime?'':'disabled'}><small id="neuralRateValue">${esc(neuralRateLabel(selectedNeuralRate()))}</small></label><div class="indonesian-voice-block"><p id="indonesianVoiceStatus">${esc(indonesianVoiceLabel())}</p><div class="voice-setup-actions"><button id="testIndonesianVoice" type="button" ${indonesianPrepared()?'':'disabled'}><i data-lucide="volume-2"></i> ${esc(tr('voice.id.test'))}</button><button id="prepareIndonesianVoice" type="button" ${self.FiezelIndonesianVoice?'':'disabled'}><i data-lucide="download"></i> ${esc(tr(indonesianPrepared()?'voice.id.verify':'voice.id.prepare'))}</button></div></div></div><div class="voice-setup-actions"><button id="testNeuralVoice" type="button" ${runtime&&status.prepared?'':'disabled'}><i data-lucide="volume-2"></i> ${esc(tr('voice.test'))}</button><button id="prepareNeuralVoice" class="primary" type="button" ${runtime?'':'disabled'}><i data-lucide="download"></i> ${esc(tr(status.ready?'voice.verify':'voice.prepare'))}</button></div></article>`}
...
function vocab(){const counts=Object.fromEntries(LEVELS.map(l=>[l,0]));V.forEach(v=>counts[v.level]=(counts[v.level]||0)+1);shell(tr('vocab.title'),tr('vocab.subtitle',{n:V.length.toLocaleString()}),`<div class="toolbar"><button class="primary" onclick="startVocabQuiz()" data-testid="vocab-quiz-start"><i data-lucide="circle-play"></i> ${esc(tr('vocab.quiz'))}</button><button onclick="reviewVocab()" data-testid="vocab-review-due"><i data-lucide="history"></i> ${esc(tr('vocab.reviewDue',{n:Object.values(state.vocab).filter(x=>x.nextReview&&x.nextReview<=Date.now()&&x.mastery<80).length}))}</button></div><div class="grid">${LEVELS.map(l=>card(`<div class="row"><b>${l}</b><span>${esc(tr('vocab.words',{n:counts[l]||0}))}</span></div><p class="muted">${esc(tr('vocab.masteredCount',{n:Object.entries(state.vocab).filter(([id,x])=>V.find(v=>v.id===id)?.level===l&&x.mastery>=80).length}))}</p>${counts[l]?`<button onclick="flashcards('${l}')" data-testid="vocab-flashcards-${l}">${esc(tr('vocab.openFlashcards'))} <i data-lucide="arrow-right"></i></button>`:`<p class="muted">${esc(tr('vocab.unavailable'))}</p>`}`)).join('')}</div>`)}
// Meaning shown in the UI language. Thai glosses live in vocabulary-th.json; when a word has no Thai gloss yet, an English synonym gloss is used so Indonesian never leaks.
function vocabMeaning(v){if(!v)return '';if(!isThai())return v.meaning||'';const th=VOCAB_TH_MEANINGS[v.id];if(th)return th;const syn=(v.synonyms||[]).slice(0,2).join(', ');return syn}
function vocabExampleTranslation(v){return isThai()?'':(v?.exampleTranslation||'')}
...
function flashcards(level){
  ...
  const draw=()=>{
    ...
    setApp(`<section class="fade"><div class="topline"><button id="backVocab" data-testid="flashcard-back"><i data-lucide="arrow-left"></i> ${esc(tr('vocab.back'))}</button><b>${i+1}/${pool.length}</b></div>${card(`<div class="flashcard ${flipped?'flipped':''}" id="flashcard" role="button" tabindex="0" data-testid="flashcard"><div class="flash-inner"><div class="flash-face flash-front"><div class="eyebrow">${esc(v.level)} · ${esc(indonesianPartOfSpeech(v.partOfSpeech))}</div><h2 class="word" data-testid="flashcard-word">${esc(v.word)}</h2><div class="phonetic">${esc(v.phonetic||tr('vocab.noPhonetic'))}</div><p class="muted">${esc(tr('vocab.tapMeaning'))}</p></div><div class="flash-face flash-back"><div class="eyebrow">${esc(tr('vocab.meaning'))}</div><h3 data-testid="flashcard-meaning">${esc(vocabMeaning(v)||tr('vocab.glossFallback'))}</h3><p>${esc(v.example)}</p>${vocabExampleTranslation(v)?`<p class="muted">${esc(vocabExampleTranslation(v))}</p>`:''}<div class="flash-actions"><button id="learning">${esc(tr('vocab.stillLearning'))}</button><button class="primary" id="mastered">${esc(tr('vocab.markMastered'))}</button></div><p class="muted">${esc(tr('vocab.tapBack'))}</p></div></div></div><div class="actions"><button id="speakWord"><i data-lucide="volume-2"></i> ${esc(tr('vocab.hearWord'))}</button><button id="speakSentence"><i data-lucide="audio-lines"></i> ${esc(tr('vocab.hearSentence'))}</button><button id="aiWord" data-testid="flashcard-ask-ai"><i data-lucide="sparkles"></i> ${esc(tr('vocab.askAI'))}</button></div><div class="swipe-hint">${esc(tr('vocab.swipeHint'))}</div><div class="notice">${esc(tr('vocab.status',{status:tr(state.vocab[v.id]?.mastery>=80?'vocab.status.mastered':state.vocab[v.id]?.total?'vocab.status.learning':'vocab.status.new')}))}</div>`)} </section>`);
    ...
    $('learning').onclick=e=>{e.stopPropagation();updateMastery('vocab',v.id,false);save();showToast(tr('result.toast.saved'))};
    $('mastered').onclick=e=>{e.stopPropagation();markMastered('vocab',v.id);haptic('success');showToast(tr('vocab.status.mastered'));i++;draw()};
...
function reviewVocab(){
  ...
  const draw=()=>{
    ...
    setApp(`<section class="fade"><div class="topline"><button id="backReview"><i data-lucide="arrow-left"></i> ${esc(tr('vocab.back'))}</button><b>${esc(tr('vocab.reviewLabel',{i:i+1,n:due.length}))}</b></div>${card(`<div class="flashcard ${flipped?'flipped':''}" id="reviewCard" role="button" tabindex="0"><div class="flash-inner"><div class="flash-face flash-front"><div class="eyebrow">${esc(tr('vocab.reviewKicker'))} · ${esc(v.level)}</div><h2 class="word">${esc(v.word)}</h2><div class="phonetic">${esc(v.phonetic||tr('vocab.noPhonetic'))}</div><p class="muted">${esc(tr('vocab.tapMeaning'))}</p></div><div class="flash-face flash-back"><div class="eyebrow">${esc(tr('vocab.meaning'))}</div><h3>${esc(vocabMeaning(v)||tr('vocab.glossFallback'))}</h3><p>${esc(v.example)}</p><div class="flash-actions"><button id="reviewLearning">${esc(tr('vocab.stillLearning'))}</button><button class="primary" id="reviewMastered">${esc(tr('vocab.markMastered'))}</button></div><p class="muted">${esc(tr('vocab.tapBack'))}</p></div></div></div><div class="swipe-hint">${esc(tr('vocab.swipeHint'))}</div>`)} </section>`);
...
function startVocabQuiz(){const pool=shuffle(V.filter(v=>v.level===LEVELS[Math.max(0,Math.min(5,(state.level||1)-1))]));const source=pool.length?pool:shuffle(V);if(!source.length)return showToast(tr('vocab.noVocab'));quizLoop({type:'vocab',count:12,pool:source,factory:makeVocabQuestion})}
function makeVocabQuestion(v){
  ...
  const posLabel=indonesianPartOfSpeech(v.partOfSpeech),gloss=meaning||(v.synonyms||[]).slice(0,2).join(', ')||v.word;const why=type==='context'?tr('vocab.why.context',{word:v.word,meaning:gloss}):type==='partOfSpeech'?tr('vocab.why.pos',{word:v.word,pos:posLabel}):type==='synonym'?tr('vocab.why.synonym',{synonym:v.synonyms[0],word:v.word}):tr('vocab.why.meaning',{word:v.word,meaning:gloss,pos:v.partOfSpeech?tr('vocab.why.meaningPos',{pos:posLabel}):''});
...
function grammar(){const skills=Object.keys(G);shell(tr('grammar.title'),tr('grammar.subtitle',{n:skills.length}),`<div class="grid grammar-grid">${skills.map(k=>{const item=G[k]?.[0]||[],family=grammarFamilyLabel(item);return card(`<div class="row"><b>${esc(friendlySkillName(k))}</b><span>${esc(item[5]||'A1')}</span></div><p class="muted">${esc(tr('grammar.focus',{family,n:GRAMMAR_SESSION_SIZE}))}</p><div class="lesson-card-foot"><span>${esc(tr('grammar.mastery',{n:state.grammar[k]?.mastery||0}))}</span><button onclick="openGrammarLesson('${esc(k)}')" data-testid="grammar-open-${esc(k)}">${esc(tr('grammar.open'))} <i data-lucide="arrow-right"></i></button></div>`)}).join('')}</div>`)}
function openGrammarLesson(skill){const arr=G[skill]||[];if(!arr.length)return showToast(tr('grammar.noContent'));const item=arr[0],base=item[0],opts=item[1]||[],correct=opts[item[2]],rule=grammarRuleIndonesian(item),clue=grammarClue(base);shell(friendlySkillName(skill),tr('grammar.lessonSub',{level:item[5]||'A1',n:GRAMMAR_SESSION_SIZE}),`${card(`<div class="eyebrow">${esc(tr('grammar.understandFirst'))}</div><h2 class="lesson-title" data-testid="grammar-lesson-title">${esc(tr('grammar.gist'))}</h2><p>${esc(rule)}</p><div class="lesson-example"><span>${esc(tr('grammar.example'))}</span><h3>${esc(base)}</h3><p>${tr('grammar.fits',{answer:esc(correct),clue:esc(clue)})}</p></div><p class="memory-tip"><i data-lucide="lightbulb"></i><span>${esc(tr('grammar.tip'))}</span></p>`,'grammar-lesson-card')}<div class="practice-contract"><div><h3>${esc(tr('grammar.modesTitle',{n:GRAMMAR_SESSION_SIZE}))}</h3><p>${esc(tr('grammar.modesBody'))}</p></div><button onclick="practiceSkill('${esc(skill)}')" class="primary" data-testid="grammar-practice-start">${esc(tr('grammar.start',{n:GRAMMAR_SESSION_SIZE}))} <i data-lucide="arrow-right"></i></button></div><div class="toolbar"><button onclick="go('grammar')"><i data-lucide="arrow-left"></i> ${esc(tr('grammar.backHub'))}</button></div>`)}
...
function makeReadingQuestion(r,q,i){
  ...
  const stem=pick(readingStems(type))||readingStems('detail')[0];
  ...
  return{id:`reading-${r.id}-${i}-${Date.now()}-${Math.random()}`,type:'reading',skill:`reading_${type}`,target:r.id,difficulty:LEVELS.indexOf(r.level)+1,canary:meta.__fiezelCanary||null,passage:{id:r.id,title:r.title||tr('reading.titleDefault'),text:r.text||''},question:contextualStem,options:shuffled.map(x=>x.x),answerIndex,explain:{evidence,why:evidence?tr('reading.why.evidence',{evidence}):tr('reading.why.default'),rule:tr('reading.rule',{focus}),avoid:tr('reading.avoid'),memory:tr('reading.memory'),distractor:tr('reading.distractor')}}
}
...
function finishQuiz(cfg,score,total){
  ...
  setApp(`<section class="fade center result-stage">${card(`<div class="result-icon"><i data-lucide="trophy"></i></div><div class="modal-mark">${esc(tr('result.mark'))}</div><h2 data-testid="result-title">${esc(tr(cfg.placement?'result.placementDone':'result.practiceDone'))}</h2><div class="score" data-testid="result-score">${accuracy}%</div><p>${esc(tr('result.score',{score,total}))}</p>${outcomeLine}<button class="primary" onclick="go('home')" data-testid="result-home">${esc(tr('result.home'))} <i data-lucide="arrow-right"></i></button>`,'hero result-card')}</section>`);
  showToast(tr(accuracy>=70?'result.toast.strong':'result.toast.saved'));
...
function progress(){
 const acc=state.totalAnswered?Math.round(state.totalCorrect/state.totalAnswered*100):0;const profile=getDiagnosticProfile();const map=[[tr('progress.domain.vocab'),Object.values(state.vocab)],[tr('progress.domain.grammar'),Object.values(state.grammar)],[tr('progress.domain.reading'),Object.values(state.reading)]];
 const mapCards=map.map(([name,items])=>{const active=items.filter(x=>x?.total);const m=active.length?Math.round(active.reduce((a,x)=>a+(x.mastery||0),0)/active.length):0;return card(`<div class="row"><b>${name}</b><strong>${m}%</strong></div><div class="bar"><i style="width:${m}%"></i></div><p class="muted">${esc(tr('progress.evidenceCount',{n:active.length}))}</p>`) }).join('');
 const tl=skillTimeline(),days=Object.keys(tl).sort().slice(-7);const timelineHtml=days.length?days.map(d=>{const x=tl[d];const vals=Object.entries(x).map(([k,v])=>`${esc(friendlySkillName(k))} ${Math.round(v.correct/v.total*100)}%`).join(' · ');return `<div class="timeline-row"><span>${d}</span><span>${vals}</span></div>`}).join(''):`<p class="muted">${esc(tr('progress.noTimeline'))}</p>`;
 ...
 const readingHtml=readingSkills.map(k=>{const hs=(state.history||[]).filter(h=>h.skill===k);const a=hs.length?Math.round(hs.filter(h=>h.ok).length/hs.length*100):null;return `<div class="meter"><div class="row"><span>${esc(readingFocusLabel(k.replace('reading_','')))}</span><span>${a==null?'—':a+'%'} · ${esc(tr('progress.evidence',{n:hs.length}))}</span></div><div class="bar"><i style="width:${a||0}%"></i></div></div>`}).join('');
 ...
 ${card(`<div class="row"><b>${esc(tr('progress.creator'))}</b><a href="https://instagram.com/fitrarustqi" target="_blank" rel="noopener noreferrer" class="creator-link"><img src="./instagram.svg" alt="Instagram" width="22" height="22"> @fitrarustqi</a></div>`)}
...
function aiErrorMessage(err){const code=String(err?.error||err?.code||'').toLowerCase();if(code==='popup_blocked')return tr('ai.err.popup');if(code==='auth_window_closed')return tr('ai.err.closed');if(err?.name==='TimeoutError'||code==='timeout')return tr('ai.err.timeout',{s:FIEZEL_AI_TIMEOUT_MS/1000});const raw=err?.message||err?.msg||err?.error_description||err?.error||err;if(typeof raw==='string'&&raw.trim())return raw;try{const text=JSON.stringify(raw);return text&&text!=='{}'?text:tr('ai.err.unavailable')}catch{return tr('ai.err.unavailable')}}
async function askFiezelAI(prompt){
  if(CORE_AI_GATEWAY!=='core-only')throw new Error(tr('ai.err.config'));
  if(typeof puter==='undefined'||!puter?.workers?.exec)throw new Error(tr('ai.err.notReady'));
  if(!CORE_WORKER_URL)throw new Error(tr('ai.err.noCore'));
  let timer;try{
    const request=coreWorkerExec('/api/ai/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt})}).then(async r=>{let data={};try{data=await r.json()}catch{}if(!r.ok||!data?.text)throw new Error(data?.error||tr('ai.err.status',{status:r.status}));return data.text});
    const timeout=new Promise((_,reject)=>{timer=setTimeout(()=>{const e=new Error(tr('ai.err.timeoutShort'));e.name='TimeoutError';reject(e)},FIEZEL_AI_TIMEOUT_MS)});
    const res=await Promise.race([request,timeout]);if(typeof res==='string'&&res.trim())return res;throw new Error(tr('ai.err.noText'));
  }finally{clearTimeout(timer)}
}
function openAILoading(title){return openModal(`<div class="modal-mark">${esc(tr('ai.mark'))}</div><h2>${esc(title)}</h2><p>${esc(tr('ai.loadingBody'))}</p><div class="ai-loading" role="status" aria-live="polite" aria-label="${esc(tr('ai.loadingAria'))}" data-testid="ai-loading"><span></span><span></span><span></span></div>`)}
function renderAIResult(title,text){$('modalPanel').innerHTML=`<div class="modal-mark">${esc(tr('ai.mark'))}</div><h2>${esc(title)}</h2><div class="ai-answer" data-testid="ai-answer"><p>${esc(text).replace(/\n/g,'<br>')}</p></div><div class="modal-actions"><button class="primary" id="aiClose">${esc(tr('ai.close'))}</button></div>`;$('aiClose').onclick=closeModal;enhanceUI()}
function renderAIError(title,err,retry){$('modalPanel').innerHTML=`<div class="modal-mark">${esc(tr('ai.mark'))}</div><h2>${esc(title)}</h2><p data-testid="ai-error">${esc(tr('ai.errorBody'))}</p><p class="muted">${esc(aiErrorMessage(err))}</p><div class="modal-actions">${retry?`<button id="aiRetry">${esc(tr('ai.retry'))}</button>`:''}<button class="primary" id="aiClose">${esc(tr('ai.close'))}</button></div>`;$('aiClose').onclick=closeModal;if(retry)$('aiRetry').onclick=retry;enhanceUI()}
...
function renderCoachResult(text){$('modalPanel').innerHTML=`<div class="modal-mark">${esc(tr('ai.coachMark'))}</div><h2>${esc(tr('ai.coachResultTitle'))}</h2><div class="ai-answer coach-answer"><p>${esc(text).replace(/\n+/g,'</p><p>')}</p></div><p class="ai-disclosure"><i data-lucide="shield-check"></i> ${esc(tr('ai.coachDisclosure'))}</p><div class="modal-actions"><button id="coachMap">${esc(tr('ai.coachMap'))}</button><button class="primary" id="coachStart">${esc(tr(state.adaptiveReady?'ai.coachStartAdaptive':'ai.coachStartDiagnostic'))}</button></div>`;$('coachMap').onclick=()=>{closeModal();go('progress')};$('coachStart').onclick=()=>{closeModal();state.adaptiveReady?startAdaptive():go('test')};enhanceUI()}
async function askCoachAI(){const id=++aiRequestSeq,epoch=openAILoading(tr('ai.coachLoading'));const snapshot=buildLearningSnapshot(),evidence=remoteLearnerEvidenceSnapshot(),policy=buildAdaptivePolicy(),outcomes=recentPolicyOutcomes(5);try{if(!CORE_WORKER_URL)throw new Error(tr('ai.err.coachNoCore'));const r=await coreWorkerExec('/api/coach/context',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({snapshot,evidence,policy,outcomes})});let data={};try{data=await r.json()}catch{}if(!r.ok||!data?.text)throw new Error(data?.error||tr('ai.err.coachStatus',{status:r.status}));if(String(data.protocol||'')!==CORE_PROTOCOL_VERSION)throw new Error('coach_protocol_mismatch');const text=String(data.text);if(currentAIRequest(id,epoch)){state.coachCache={at:Date.now(),text,snapshotAttempts:snapshot.totalAttempts,policyId:String(policy.policyId||''),outcomeId:String(outcomes.at(-1)?.outcomeId||'')};save();renderCoachResult(text)}}catch(e){if(currentAIRequest(id,epoch))renderAIError(tr('ai.coachTitle'),e,askCoachAI)}}
async function explainWithAI(q,selectedIndex){const id=++aiRequestSeq,epoch=openAILoading(tr('ai.explainTitle'));const level=LEVELS[Math.max(0,Math.min(5,(Number(q.difficulty)||1)-1))],profile=aiProfileContext();const prompt=tr('ai.prompt.explain',{level,style:aiStyle(),profile:JSON.stringify(profile),question:q.question,options:(q.options||[]).join(', '),selected:q.options?.[selectedIndex]||'-',correct:q.options?.[q.answerIndex]||'-',rule:q.explain?.rule||'-'});try{const text=await askFiezelAI(prompt);if(currentAIRequest(id,epoch))renderAIResult(tr('ai.explainTitle'),text)}catch(e){if(currentAIRequest(id,epoch))renderAIError(tr('ai.explainTitle'),e,()=>explainWithAI(q,selectedIndex))}}
async function explainWordWithAI(v){const id=++aiRequestSeq,epoch=openAILoading(v.word),profile=aiProfileContext();const prompt=tr('ai.prompt.word',{level:v.level||tr('ai.level.beginner'),style:aiStyle(),profile:JSON.stringify(profile),word:v.word,meaning:vocabMeaning(v)||tr('ai.meaningUnknown'),example:v.example});try{const text=await askFiezelAI(prompt);if(currentAIRequest(id,epoch))renderAIResult(v.word,text)}catch(e){if(currentAIRequest(id,epoch))renderAIError(v.word,e,()=>explainWordWithAI(v))}}
function resetProgress(){openModal(`<div class="modal-mark">FIEZEL</div><h2>${esc(tr('reset.title'))}</h2><p>${esc(tr('reset.body'))}</p><div class="modal-actions"><button id="modalCancel">${esc(tr('reset.cancel'))}</button><button class="primary danger" id="modalOk">${esc(tr('reset.confirm'))}</button></div>`);$('modalCancel').onclick=closeModal;$('modalOk').onclick=()=>{localStorage.removeItem('fiezel-v4-state');state=loadState();closeModal();go('home');showToast(tr('reset.done'))}}
