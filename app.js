...
// m025-180: `brain` ikut dikirim ke jalur pelatih, ringkasan yang SAMA yang dikirim ke
// /api/policy/next. Sejak rilis ini worker menghitung ulang kebijakan yang dijelaskan pelatih
// lewat jalan yang sama dengan kebijakan yang diterima murid - dan jalan itu membaca `brain`.
// Tanpa baris ini, dua rute yang seharusnya sepakat justru berangkat dari masukan berbeda:
// kebijakan murid berdiri di atas ringkasan perangkat ini, sedangkan yang dijelaskan pelatih
// berdiri di atas cermin server, dan pelatih akan menjelaskan rencana yang bukan rencananya.
// Kebijakan lokal tetap ikut dikirim seperti sebelumnya, tetapi worker TIDAK memercayainya:
// ia menghitung ulang, karena kebijakan kiriman klien adalah masukan yang tidak tepercaya.
async function askCoachAI(){const id=++aiRequestSeq,epoch=openAILoading(personalize('Menganalisis skill {name}'));const snapshot=buildLearningSnapshot(),evidence=remoteLearnerEvidenceSnapshot(),policy=buildAdaptivePolicy(),outcomes=recentPolicyOutcomes(5),profile=aiProfileContext(),brain=coreBrainDigest();try{
