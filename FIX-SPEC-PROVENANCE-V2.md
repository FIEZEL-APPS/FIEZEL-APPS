...
A7. Validasi diri: `node --check app.js` lalu jalankan: node grammar-memory-scope-test.js; node grammar-unlock-test.js; node grammar-quality-audit.js; node experience-integration-test.js; node regression-test.js. Semua harus PASS/exit 0 (audit boleh FAIL sementara pada check baru milik Subagent B jika B sudah menulis — koordinasi lewat file, jangan edit file milik B).
...
### Harness verifikasi (Subagent C) — file BARU grammar-provenance-verify.js + dump review
C1. Boot app.js via vm mengikuti pola persis grammar-memory-scope-test.js (stub DOM, fetch file lokal, tunggu load selesai).
...
- Subagent C: grammar-provenance-verify.js (baru), folder /home/user/workspace/review/ SAJA.
