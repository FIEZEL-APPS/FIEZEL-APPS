...
// Diagnostik BadWebPushTopic: log kegagalan hanya mencetak statusCode dan body,
// jadi tidak ada cara membedakan layanan push mana yang menolak. Endpoint langganan adalah
// data murid - host-nya saja sudah cukup untuk membedakan Apple/FCM/Mozilla, dan sisanya
// (path + kunci) tidak pernah ikut tercetak.
const endpointHost=(sub)=>{try{return new URL(String(sub?.endpoint||'')).host}catch{return 'unparseable'}};
...
for(const item of payload.due||[]){
  ...
  const sentTopic=safeTopic(notification.tag);
  let status='sent';try{await webpush.sendNotification(item.subscription,JSON.stringify(notification),{TTL:86400,urgency:'normal',topic:sentTopic});sent++}catch(e){status=(e.statusCode===404||e.statusCode===410)?'expired':`failed:${e.statusCode||'error'}`;failed++;console.error('push failed',e.statusCode||'',e.body||e.message,JSON.stringify({host:endpointHost(item.subscription),kind:String(notification.kind||''),tag:String(notification.tag||''),topic:sentTopic,topicLength:sentTopic.length}))}
