...
for(const item of payload.due||[]){
  ...
  let status='sent';try{await webpush.sendNotification(item.subscription,JSON.stringify(notification),{TTL:86400,urgency:'normal',topic:String(notification.tag||'fiezel').slice(0,32)});sent++}catch(e){status=(e.statusCode===404||e.statusCode===410)?'expired':`failed:${e.statusCode||'error'}`;failed++;console.error('push failed',e.statusCode||'',e.body||e.message)}
