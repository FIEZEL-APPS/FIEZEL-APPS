...
`core-brain-v3-upgrade-test.js`, `tutor-brain-v3-test.js`, `brain-manifest-test.js`,
`brain-config-test.js`, `mastery-bkt-test.js`, `olm-test.js`, `item-calibration-test.js`,
`srl-coach-test.js`. (Delapan berkas ini, bukan seluruh 253 invokasi test di
...
dinilai murid**. `brain-page-wiring-test.js` tidak menangkapnya karena ia hanya menguji arah
...
- Tambah gerbang arah-kedua di `brain-page-wiring-test.js`: modul berotoritas `off` yang
...
`brain-manifest-test.js` + gerbang baru hijau. Ritual bump `SW_REV`/`DIAG_BUILD`/`FIEZEL_PAGE_BUILD` ikut.
...
- Kontrak sinkronisasi `brain-config-test.js` (default wajib identik dengan konstanta sumber)
