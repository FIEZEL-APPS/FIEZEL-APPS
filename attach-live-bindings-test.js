...
const TOML = require('fs').readFileSync(path.join(__dirname, 'workers', 'api', 'wrangler.toml'), 'utf8');
...
function loadCore() {
  const script = `
    import { computeAttachedToml, AttachError } from ${JSON.stringify(path.join(__dirname, 'workers', 'api', 'tools', 'attach-live-bindings-core.mjs'))};
  ...
  return import(path.join(__dirname, 'workers', 'api', 'tools', 'attach-live-bindings-core.mjs'));
}
