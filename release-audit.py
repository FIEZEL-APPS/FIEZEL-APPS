...
check('Grammar schema contract',GM.get('version')=='5.18.0' and GM.get('schemaVersion')=='2.0.0' and GM.get('practiceBlueprintVersion')=='focused-25-v1',f"version={GM.get('version')} schema={GM.get('schemaVersion')} blueprint={GM.get('practiceBlueprintVersion')}")
# Reading
...
# PWA
manifest=json.load(open(ROOT/'manifest.json')); version=json.load(open(ROOT/'VERSION.json'))['version']; sw=open(ROOT/'sw.js').read(); idx=open(ROOT/'index.html').read()
