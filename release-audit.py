...
check('Remote Web Push architecture','ensureRemotePushSubscription' in app and "addEventListener('push'" in (ROOT/'sw.js').read_text(encoding='utf-8',errors='ignore') and 'me.puter.kv' in (ROOT/'fiezel-core-worker.js').read_text(encoding='utf-8',errors='ignore') and 'webpush.sendNotification' in (ROOT/'push-dispatcher.mjs').read_text(encoding='utf-8',errors='ignore'),'Push API subscription, service-worker receive path, Puter backend, and VAPID dispatcher are present')
check('Remote push secret separation','VAPID_PRIVATE_KEY' not in app and 'VAPID_PRIVATE_KEY' not in (ROOT/'fiezel-core-worker.js').read_text(encoding='utf-8',errors='ignore') and 'VAPID_PRIVATE_KEY' in (ROOT/'push-dispatcher.mjs').read_text(encoding='utf-8',errors='ignore'),'VAPID private key is scheduler-only and not bundled in browser or Puter Worker runtime')
...
check('Adaptive policy runtime contract','fiezel-adaptive-policy-v1' in app and 'resolveAdaptivePolicy' in app and '/api/policy/next' in (ROOT/'fiezel-core-worker.js').read_text(encoding='utf-8',errors='ignore'),'deterministic policy derives next-best action and Core exposes authenticated policy endpoint')
worker_core=(ROOT/'fiezel-core-worker.js').read_text(encoding='utf-8',errors='ignore')
...
check('Content QA runtime contract',(ROOT/'content-qa-agent.js').exists() and (ROOT/'tests/content-qa-agent-test.js').exists() and "CONTENT_QA_SCHEMA='fiezel-content-qa-v1'" in worker_core and '/api/content/qa/review' in worker_core and "authority:'advisory-only'" in worker_core and '/api/content/qa/apply' not in worker_core,'deterministic read-only scan plus bounded owner-only advisory AI review; no mutation endpoint')
check('Guarded Content Patch runtime contract',(ROOT/'content-patch-gate.js').exists() and (ROOT/'tests/content-patch-gate-test.js').exists() and "CONTENT_PATCH_SCHEMA='fiezel-content-patch-v1'" in worker_core and '/api/content/patch/candidate' in worker_core and "authority:'candidate-only'" in worker_core and 'UNVERIFIED_LOCAL_GATES_REQUIRED' in worker_core and '/api/content/patch/apply' not in worker_core and '/api/content/patch/publish' not in worker_core,'AI output is candidate-only; source-hash local gate owns schema/answer/pedagogy/regression validation and no mutation/publish endpoint exists')
...
check('Canonical Adoption release-only contract',(ROOT/'content-adoption.js').exists() and (ROOT/'tests/content-adoption-test.js').exists() and 'fiezel-content-adoption-v1' in (ROOT/'content-adoption.js').read_text(encoding='utf-8',errors='ignore') and './content-adoption.js' not in (ROOT/'index.html').read_text(encoding='utf-8',errors='ignore') and 'content-adoption.js' not in (ROOT/'sw.js').read_text(encoding='utf-8',errors='ignore') and '/api/content/adoption' not in worker_core and '/api/content/patch/apply' not in worker_core and '/api/content/patch/publish' not in worker_core,'owner/audit-gated canonical adoption is release-time tooling only; no browser/Core canonical mutation route exists')
check('Adoption Evidence Attestation release-only contract',(ROOT/'content-adoption-evidence.js').exists() and (ROOT/'tests/content-adoption-evidence-test.js').exists() and 'fiezel-adoption-evidence-attestation-v1' in (ROOT/'content-adoption-evidence.js').read_text(encoding='utf-8',errors='ignore') and 'content-adoption-evidence.js' not in (ROOT/'index.html').read_text(encoding='utf-8',errors='ignore') and 'content-adoption-evidence.js' not in (ROOT/'sw.js').read_text(encoding='utf-8',errors='ignore') and '/api/content/adoption/evidence' not in worker_core,'aggregate evidence attestation is deterministic release-time tooling only; no learner/browser/Core export endpoint exists')
check('Production Evidence Origin release-only contract',(ROOT/'content-evidence-origin.js').exists() and (ROOT/'tests/content-evidence-origin-test.js').exists() and 'fiezel-evidence-origin-envelope-v1' in (ROOT/'content-evidence-origin.js').read_text(encoding='utf-8',errors='ignore') and 'content-evidence-origin.js' not in (ROOT/'index.html').read_text(encoding='utf-8',errors='ignore') and 'content-evidence-origin.js' not in (ROOT/'sw.js').read_text(encoding='utf-8',errors='ignore') and '/api/content/evidence/origin' not in worker_core,'signed production origin verification is release-time only and requires external operator trust policy')
check('Operator Adoption Rehearsal release-only contract',(ROOT/'content-adoption-rehearsal.js').exists() and (ROOT/'tests/content-adoption-rehearsal-test.js').exists() and 'fiezel-content-adoption-rehearsal-v1' in (ROOT/'content-adoption-rehearsal.js').read_text(encoding='utf-8',errors='ignore') and 'content-adoption-rehearsal.js' not in (ROOT/'index.html').read_text(encoding='utf-8',errors='ignore') and 'content-adoption-rehearsal.js' not in (ROOT/'sw.js').read_text(encoding='utf-8',errors='ignore') and '/api/content/adoption/rehearsal' not in worker_core,'rehearsal is local release tooling with staging/rollback verification and no runtime/Core mutation endpoint')
...
activation_ok=all((ROOT/f).exists() for f in activation_files) and '@heyputer/cli@0.1.2' in (ROOT/'.github/workflows/deploy-core-worker.yml').read_text(encoding='utf-8',errors='ignore')
...
worker=(ROOT/'fiezel-report-worker.js').read_text(encoding='utf-8',errors='ignore')
...
core_worker_contract_run=subprocess.run(['node',str(ROOT/'tests/core-worker-contract-test.js')],capture_output=True,text=True)
