...
check('Shadow/Canary runtime contract',(ROOT/'content-canary.js').exists() and (ROOT/'content-canary-config.js').exists() and (ROOT/'content-canary-config-builder.js').exists() and (ROOT/'content-canary-test.js').exists() and 'fiezel-content-canary-v1' in (ROOT/'content-canary.js').read_text(encoding='utf-8',errors='ignore') and 'CONTENT_CANARY.prepare' in app and 'CONTENT_CANARY.recordEvidence' in app and './content-canary.js' in (ROOT/'index.html').read_text(encoding='utf-8',errors='ignore') and './content-canary-config.js' in (ROOT/'index.html').read_text(encoding='utf-8',errors='ignore'),'gated candidate overlay is isolated, bounded by cohort/session/expiry, records aggregate evidence, and falls back to immutable canonical data')
...
check('Canonical Adoption release-only contract',(ROOT/'content-adoption.js').exists() and (ROOT/'content-adoption-test.js').exists() and 'fiezel-content-adoption-v1' in (ROOT/'content-adoption.js').read_text(encoding='utf-8',errors='ignore') and './content-adoption.js' not in (ROOT/'index.html').read_text(encoding='utf-8',errors='ignore') and 'content-adoption.js' not in (ROOT/'sw.js').read_text(encoding='utf-8',errors='ignore') and '/api/content/adoption' not in worker_core and '/api/content/patch/apply' not in worker_core and '/api/content/patch/publish' not in worker_core,'owner/audit-gated canonical adoption is release-time tooling only; no browser/Core canonical mutation route exists')
check('Adoption Evidence Attestation release-only contract',(ROOT/'content-adoption-evidence.js').exists() and (ROOT/'content-adoption-evidence-test.js').exists() and 'fiezel-adoption-evidence-attestation-v1' in (ROOT/'content-adoption-evidence.js').read_text(encoding='utf-8',errors='ignore') and 'content-adoption-evidence.js' not in (ROOT/'index.html').read_text(encoding='utf-8',errors='ignore') and 'content-adoption-evidence.js' not in (ROOT/'sw.js').read_text(encoding='utf-8',errors='ignore') and '/api/content/adoption/evidence' not in worker_core,'aggregate evidence attestation is deterministic release-time tooling only; no learner/browser/Core export endpoint exists')
check('Production Evidence Origin release-only contract',(ROOT/'content-evidence-origin.js').exists() and (ROOT/'content-evidence-origin-test.js').exists() and 'fiezel-evidence-origin-envelope-v1' in (ROOT/'content-evidence-origin.js').read_text(encoding='utf-8',errors='ignore') and 'content-evidence-origin.js' not in (ROOT/'index.html').read_text(encoding='utf-8',errors='ignore') and 'content-evidence-origin.js' not in (ROOT/'sw.js').read_text(encoding='utf-8',errors='ignore') and '/api/content/evidence/origin' not in worker_core,'signed production origin verification is release-time only and requires external operator trust policy')
