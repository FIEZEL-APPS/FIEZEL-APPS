# FIEZEL m025-201 — F1: jebakan jangkar censoring mastery

**Build:** m025-201 · **Basis:** 5.19.0 · **Tanggal:** 2026-08-30 · **PR:** #253
**Otoritas:** keputusan trade-off di bawah adalah **keputusan OWNER/MASTER**, diambil
2026-08-30. Wave tidak berwenang memutuskannya sendiri — aturan itu ditulis di
...
- `momentum()` basis residual mengekspor **`residualPositiveShare`** — porsi blok jendela yang
  mean residualnya positif (0..1). Dipilih ketimbang streak mentah karena streak jatuh ke nol
  oleh satu blok derau, dan varian streak≥3 terbukti menaikkan osilasi 2.98→4.08 per 10 sesi
  (gagal gate keras vs v1).
- `planSession`: cabang baru — bila `state !== 'declining'` dan `residualPositiveShare >= 0.75`,
...
tanpa field baru → perilaku lama utuh.
...
tertinggal dari kemampuan nyata, informasi Fisher mengecil, θ̂ makin lambat menyusul, jangkar
tidak pernah naik.

## Trade-off yang diterima OWNER, dengan angkanya

50 seed × 9 profil, CI bootstrap berpasangan, sebelum → sesudah pada kebijakan shipped:

| Metrik | Sebelum | Sesudah | ΔCI 95% | Arah |
|---|---|---|---|---|
| censoredRate | 0.8711 | 0.6489 | [−0.260, −0.184] | lebih baik |
| timeToMastery (58 pasangan tak tersensor) | 29.0 h | 20.7 h | [−9.62, −6.97] | lebih baik |
| falseDeclineRate (seed 42, konstrain ≤ 0.010) | 0.0101 | 0.0000 | — | terpenuhi |
| retentionDay90 | 0.3618 | 0.3204 | [−0.050, −0.033] | **lebih buruk** |
...
mekanis — akurasi turun 0.80→0.74 menaikkan lantai p(1−p) 0.159→0.191, jadi margin kalibrasi
...
m025-200 → **m025-201** lewat `node tools/bump-build.mjs`. Wajib, bukan kosmetik: A7 menuntut
`DIAG_BUILD = base+1` setiap kali berkas berpola `features/` berubah, dan F1 mengubah
`fiezel-core-brain.js` yang ikut precache. `SW_REV` membawa prefiks `m025-201-` supaya PWA
...
1. `voice-fallback-chain-test.js` — "tangga suara NOT READY (42 pass, 3 fail)". **Bukan bug
   ...
   test-nya, yang tidak melakukan preload i18n — kelas kerusakan yang sama sudah pernah
   diperbaiki untuk 10 harness lain.
2. `content-drift-test.js` — assert konsistensi-diri gerbang wave F2: alat perbaikan
...
1. Perbaiki dua gerbang merah di `main` di atas — keputusan arah untuk nomor 2 ada di OWNER
   kalau ternyata gerbangnya yang salah, bukan datanya.
2. Merge PR #253 setelah `quality` hijau.
3. Merge PR #261 (arbiter nomor build) — terhalang oleh dua gerbang yang sama.
