#!/usr/bin/env node
/**
 * GERBANG HITUNG-ULANG SINKRON (brain-sync-rebuild-test.js)
 *
 * INI PANEN DARI BUKTI S4, DAN GERBANG YANG MENJAGANYA TETAP BENAR
 * ----------------------------------------------------------------
 * brain-replay-equivalence-test.js membuktikan sifat matematisnya: model otak adalah fungsi
...
test('gate ini terdaftar di CI', () => {
  ...
  assert.ok(workflow.includes('node brain-sync-rebuild-test.js'), 'gate yang tidak berjalan di CI bukan gate');
});
