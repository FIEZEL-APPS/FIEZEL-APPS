...
(function (root) {
  ...
  function openAssignment(id) {
    ...
    if (!a) { var done = subs().filter(function (x) { return x.id === id; })[0]; if (done) { ui().tab = 'tugas'; ui().review = id; saveUi(); renderStudent(); return true; } if (sEnv.toast) sEnv.toast('Tugas ini tidak ditemukan atau sudah selesai.'); return false; }
  ...
  function closeRunner() { var u = ui(); if (u.runner && !u.runner.finished && sEnv.toast) sEnv.toast('Tugas disimpan sebagai "sedang mengerjakan". Lanjutkan kapan saja.'); if (u.runner && u.runner.finished) u.runner = null; u.paused = !!u.runner; saveUi(); renderStudent(); }
  ...
  function renderStudent() {
    ...
    sEl.innerHTML = '<section class="ch ch-student" data-testid="class-hub-student">' +
      '<header class="ch-head"><div><p class="ch-kicker">Kelas</p><h1 data-testid="class-hub-title">' + esc(className() || (classCode() ? 'Kelas ' + classCode() : 'Belum terhubung ke kelas')) + '</h1><p class="ch-sub">' + (teacherName() ? 'Guru: <b>' + esc(teacherName()) + '</b> · ' : '') + (classCode() ? 'Kode ' + esc(classCode()) : 'Masukkan kode kelas dari gurumu di tab Kelas Saya') + '</p></div></header>' +
      (u.runner && !u.paused ? '' : '<nav class="ch-tabs" role="tablist">' + [['tugas', 'Tugas', pend.length], ['kelas', 'Kelas Saya', 0], ['progres', 'Progres', 0]].map(function (t) { return '<button type="button" role="tab" class="ch-tab' + (u.tab === t[0] && !u.review ? ' is-active' : '') + '" data-ch="tab" data-tab="' + t[0] + '" data-testid="class-tab-' + t[0] + '">' + t[1] + (t[2] ? '<span class="ch-badge">' + t[2] + '</span>' : '') + '</button>'; }).join('') + '</nav>') +
      body + '</section>';
  ...
  function kelasView() {
    ...
    return '<div class="ch-body"><section class="ch-card ch-class-card" data-testid="class-my-class">' + (classCode() ? '<p class="ch-kicker">Kelas terhubung</p><h3>' + esc(className() || 'Kelas ' + classCode()) + '</h3><p class="ch-muted">Kode kelas <b class="ch-mono">' + esc(classCode()) + '</b>' + (teacherName() ? ' · Guru <b>' + esc(teacherName()) + '</b>' : '') + '</p><p class="ch-muted ch-small">' + (rep ? (rep.ok ? icon('check') + ' Laporan terakhir terkirim ke guru ' + esc(fmtDate(rep.at)) : icon('clock') + ' Laporan terakhir belum terkirim (' + esc(rep.error || 'offline') + ') — dikirim ulang otomatis saat online.') : 'Setiap tugas yang selesai dikirim ke guru sebagai ringkasan: nama depan, akurasi per skill, dan soal yang keliru. Tanpa transkrip.') + '</p><div class="ch-actions"><button type="button" class="ch-btn is-ghost" data-ch="resend">' + icon('refresh-cw') + ' Kirim ulang laporan</button><button type="button" class="ch-btn is-ghost" data-ch="change-code">Ganti kode</button></div>' : '<p class="ch-kicker">Gabung kelas</p><h3>Masukkan kode dari gurumu</h3><p class="ch-muted">Kode berbentuk FZ-XXXXXX. Setelah tergabung, tugas guru masuk otomatis dan hasilmu kembali ke guru.</p>') +
      ...
      '<button type="button" class="ch-card ch-link-card" data-ch="learn" data-testid="class-open-learn"><span class="ch-link-icon">' + icon('route') + '</span><div><b>Belajar mandiri hari ini</b><small>Rencana harian dari peta kemampuanmu — tugas guru ikut masuk ke sana.</small></div>' + icon('arrow-up-right') + '</button></section></div>';
  }
  ...
  function runnerView() {
    var r = ui().runner;
    if (r.finished) { var res = r.result; return '<div class="ch-body"><section class="ch-card ch-result" data-testid="class-result"><p class="ch-kicker">Selesai</p><h2>' + esc(res.title) + '</h2><p class="ch-score">' + res.c + '<small>/' + res.t + '</small></p><p class="ch-muted">' + icon('send') + ' Hasil ini dikirim ke <b>' + esc(res.teacher || 'guru') + '</b> — termasuk soal yang perlu diulang.</p><div class="ch-actions"><button type="button" class="ch-btn is-primary" data-ch="review" data-id="' + esc(r.aid) + '">Lihat pembahasan</button><button type="button" class="ch-btn is-ghost" data-ch="close-runner">Kembali ke Tugas</button></div></section></div>'; }
  ...
  function reviewView(id) {
    ...
    return '<div class="ch-body"><button type="button" class="ch-btn is-ghost is-small" data-ch="back">' + icon('chevron-left') + ' Kembali</button><section class="ch-card"><p class="ch-kicker">Pembahasan · dari ' + esc(s.teacher || s.from || 'guru') + '</p><h2>' + esc(s.title) + '</h2><p class="ch-score">' + s.c + '<small>/' + s.t + '</small></p></section>' +
      '<ol class="ch-review-list">' + s.results.map(function (r, i) { var item = resolveItem(s, r.itemId); if (!item) return ''; var why = item.why && item.why[r.chosen]; return '<li class="ch-card ' + (r.correct ? 'is-ok' : 'is-no') + '"><p class="ch-muted">Soal ' + (i + 1) + ' · ' + esc(skillLabel(item.skill || s.skills[0])) + '</p><b>' + esc(item.prompt) + '</b><p>Jawabanmu: <span class="' + (r.correct ? 'ch-ok' : 'ch-no') + '">' + esc(item.options[r.chosen] || '—') + '</span>' + (r.correct ? '' : ' · Benar: <b>' + esc(item.options[item.answer]) + '</b>') + '</p>' + (!r.correct && (why || item.note) ? '<p class="ch-muted">' + esc(why || item.note) + '</p>' : '') + '</li>'; }).join('') + '</ol></div>';
  }
  ...
  var TABS = [['kelas', 'Kelas Saya', 'users'], ['tugas', 'Tugas', 'clipboard-list'], ['buat', 'Buat Tugas', 'plus-circle'], ['hasil', 'Hasil', 'bar-chart-3'], ['braincore', 'Braincore', 'brain']];
  ...
  function teacherMarkup(env) {
    var c = env.cls();
    var body = !c ? '<section class="ch-card ch-empty">' + icon('school') + '<p>Buat kelas dulu, lalu Kelas menjadi pusat tugas, hasil, dan insight.</p><button type="button" class="tg-btn is-primary" data-tg="modal" data-kind="new-class">Buat kelas</button></section>' : tUi.tab === 'tugas' ? tTugas(c, env) : tUi.tab === 'buat' ? tBuat(c, env) : tUi.tab === 'hasil' ? tHasil(c, env) : tUi.tab === 'braincore' ? tBraincore(c, env) : tKelas(c, env);
    return '<div class="ch ch-teacher" data-testid="class-hub-teacher"><p class="ch-principle">' + icon('brain') + ' Braincore menyarankan · Guru memutuskan · Murid belajar</p><nav class="ch-tabs is-teacher" role="tablist">' + TABS.map(function (t) { return '<button type="button" role="tab" class="ch-tab' + (tUi.tab === t[0] ? ' is-active' : '') + '" data-ch="ttab" data-tab="' + t[0] + '" data-testid="tclass-tab-' + t[0] + '">' + icon(t[2]) + '<span>' + t[1] + '</span></button>'; }).join('') + '</nav>' + body + '</div>';
  }
  ...
  function tTugas(c, env) {
    ...
    return '<div class="ch-body"><div class="ch-row ch-between"><h3 class="ch-h2">Tugas & ujian <small>' + list.length + '</small></h3><button type="button" class="tg-btn is-primary" data-ch="ttab" data-tab="buat" data-testid="tclass-new">' + icon('plus') + ' Buat tugas</button></div>' +
      (list.length ? list.map(function (a) {
        ...
        return '<article class="ch-card ch-assign' + (a.mode === 'ujian' ? ' is-exam' : '') + '" data-testid="tclass-assign-' + esc(a.id) + '"><div class="ch-card-top"><div><p class="ch-kicker">' + (a.mode === 'ujian' ? 'Ujian mini · ' + a.timer + ' mnt' : 'Latihan · ±' + a.minutes + ' mnt') + ' · ' + (a.source === 'tulis' ? 'soal guru' : a.source === 'impor' ? 'soal impor' : a.items && a.items.length ? 'campuran' : 'bank FIEZEL') + (a.review ? ' · Braincore ' + a.review.ready + '/' + a.review.count + ' siap' : '') + '</p><h3>' + esc(a.title) + '</h3></div><span class="ch-deadline' + (a.deadline && a.deadline < today() && sc.selesai < sc.total ? ' is-late' : '') + '">' + icon('calendar') + ' ' + esc(a.deadline ? 'Tenggat ' + fmtDate(a.deadline) : 'Tanpa tenggat') + '</span></div>' +
          ...
          '<div class="ch-actions"><button type="button" class="tg-btn is-small is-primary" data-tg="send-assign" data-id="' + esc(a.id) + '" data-testid="tclass-send-' + esc(a.id) + '">' + icon('send') + (sentAll ? ' Kirim ulang' : ' Kirim ke murid') + '</button><button type="button" class="tg-btn is-small is-ghost" data-ch="expand" data-id="' + esc(a.id) + '" data-testid="tclass-expand-' + esc(a.id) + '">' + icon(open ? 'chevron-up' : 'list-checks') + (open ? ' Tutup' : ' Status murid & soal') + '</button><button type="button" class="tg-btn is-small is-ghost" data-ch="result" data-id="' + esc(a.id) + '">' + icon('bar-chart-3') + ' Hasil</button><button type="button" class="tg-btn is-small is-ghost" data-tg="modal" data-kind="share-assign" data-id="' + esc(a.id) + '">' + icon('qr-code') + ' Kode</button></div>' +
          (open ? '<div class="ch-expand"><h4>Status per murid</h4><ul class="ch-mini-list">' + c.students.filter(function (s) { return TS.targeted(a, s); }).map(function (s) { var st = statusOf(a, studentRec(a, s)), d = a.done && a.done[s.id]; return '<li><span class="ch-grow">' + esc(s.name) + '</span>' + statusChip(st) + (d ? '<b>' + pct(d.acc) + '</b>' : '<button type="button" class="tg-btn is-small is-ghost" data-tg="send-assign" data-id="' + esc(a.id) + '" data-sid="' + esc(s.id) + '">' + icon('send') + '</button>') + '</li>'; }).join('') + '</ul><h4>Soal persis yang diterima murid</h4><ol class="ch-item-list" data-testid="tclass-items-' + esc(a.id) + '">' + itemList(a) + '</ol></div>' : '') + '</article>';
      }).join('') : '<section class="ch-card ch-empty">' + icon('clipboard-list') + '<p>Belum ada tugas. Susun dari bank FIEZEL, tulis sendiri, atau impor — Braincore meninjau sebelum dikirim.</p></section>') + '</div>';
  }
  ...
  function tBuat(c, env) {
    var d = draft(c), TS = T();
    var steps = '<ol class="ch-steps">' + ['Sumber soal', 'Tinjauan Braincore', 'Kirim'].map(function (s, i) { return '<li class="' + (d.step === i + 1 ? 'is-current' : d.step > i + 1 ? 'is-done' : '') + '"><span>' + (i + 1) + '</span>' + s + '</li>'; }).join('') + '</ol>';
    if (d.step === 1) {
      return '<div class="ch-body">' + steps + '<form class="ch-card ch-form-grid" data-ch-form="draft-source" data-testid="tclass-draft-form">' +
        ...
        (d.source === 'tulis' ? '<div class="ch-write" data-testid="tclass-write">' + (d.items.length ? '<ol class="ch-item-list">' + d.items.map(function (q, i) { return '<li><b>' + esc(q.prompt) + '</b><ol class="ch-opts-inline">' + q.options.map(function (o, j) { return '<li class="' + (j === q.answer ? 'is-key' : '') + '">' + esc(o) + '</li>'; }).join('') + '</ol><button type="button" class="tg-link" data-ch="drop-item" data-i="' + i + '">hapus</button></li>'; }).join('') + '</ol>' : '') +
          '<fieldset class="ch-fieldset"><legend>Soal baru</legend><label class="tg-label">Pertanyaan (gunakan ___ untuk rumpang)<input name="q_prompt" maxlength="400" placeholder="She ___ to school yesterday." data-testid="tclass-q-prompt"></label><div class="ch-form-row">' + [0, 1, 2, 3].map(function (i) { return '<label class="tg-label">' + String.fromCharCode(65 + i) + '<input name="q_opt' + i + '" maxlength="120"' + (i < 2 ? ' placeholder="wajib"' : '') + ' data-testid="tclass-q-opt' + i + '"></label>'; }).join('') + '</div><label class="tg-label">Kunci<select name="q_answer" data-testid="tclass-q-answer">' + [0, 1, 2, 3].map(function (i) { return '<option value="' + i + '">' + String.fromCharCode(65 + i) + '</option>'; }).join('') + '</select></label><button type="button" class="tg-btn is-ghost" data-ch="add-item" data-testid="tclass-add-item">' + icon('plus') + ' Tambah soal</button></fieldset></div>' : '') +
    ...
    return '<div class="ch-body">' + steps + '<section class="ch-card" data-testid="tclass-confirm"><p class="ch-kicker">Siap dikirim</p><h3>' + esc(d.title || ('Latihan ' + d.skills.map(skillLabel).join(' + '))) + '</h3><p class="ch-muted">' + ids.length + ' soal (' + (ids.length - custom.length) + ' bank FIEZEL, ' + custom.length + ' soal guru) · ' + (d.mode === 'ujian' ? 'ujian mini' : 'latihan') + ' · ' + (d.deadline ? 'tenggat ' + fmtDate(d.deadline) : 'tanpa tenggat') + ' · ' + (d.targets.length ? d.targets.length + ' murid terpilih' : 'seluruh kelas') + '</p><ol class="ch-item-list">' + itemList({ itemIds: ids, items: custom }) + '</ol>' +
      '<div class="ch-actions"><button type="button" class="tg-btn is-primary" data-ch="commit" data-send="1" data-testid="tclass-commit-send">' + icon('send') + ' Simpan & kirim ke murid</button><button type="button" class="tg-btn is-ghost" data-ch="commit" data-testid="tclass-commit-draft">Simpan saja</button><button type="button" class="tg-btn is-ghost" data-ch="step" data-step="2">' + icon('chevron-left') + ' Kembali</button></div></section></div>';
  }
  function reviewCard(r, i, d) {
    ...
    return '<article class="ch-card ch-review-card' + (approved ? ' is-approved' : '') + '" data-testid="tclass-review-' + i + '"><div class="ch-card-top"><p class="ch-kicker">Soal ' + (i + 1) + (o.fromBank ? ' · bank FIEZEL' : ' · soal guru') + '</p><label class="ch-approve"><input type="checkbox" data-ch="approve" data-i="' + i + '"' + (approved ? ' checked' : '') + ' data-testid="tclass-approve-' + i + '"> Setujui</label></div>' +
      '<div class="ch-pipeline"><section><h5>1 · Soal asli</h5><b>' + esc(o.prompt) + '</b><ol class="ch-opts-inline">' + o.options.map(function (op, j) { return '<li class="' + (j === o.answer ? 'is-key' : '') + '">' + esc(op) + '</li>'; }).join('') + '</ol></section>' +
      ...
      '<section><h5>4 · Soal final</h5>' + finalForm + '<button type="button" class="tg-link" data-ch="drop-review" data-i="' + i + '">hapus soal ini dari set</button></section></div></article>';
  }
  ...
  function tHasil(c, env) {
    var TS = T(), list = (c.assignments || []).slice().sort(function (a, b) { return b.createdAt - a.createdAt; });
    if (!list.length) return '<div class="ch-body"><section class="ch-card ch-empty">' + icon('bar-chart-3') + '<p>Hasil muncul setelah murid mengerjakan tugas. Buat tugas dulu.</p></section></div>';
    ...
    return '<div class="ch-body"><label class="tg-label">Tugas<select data-ch-select="result" data-testid="tclass-result-select">' + list.map(function (x) { return '<option value="' + esc(x.id) + '"' + (x.id === a.id ? ' selected' : '') + '>' + esc(x.title) + '</option>'; }).join('') + '</select></label>' +
      ...
      '<section class="ch-card"><p class="ch-kicker">Soal yang paling sering keliru</p>' + (im.rows.length ? '<ol class="ch-item-list" data-testid="tclass-result-items">' + im.rows.slice(0, 8).map(function (r) { return '<li><p class="ch-muted">Soal ' + (r.index + 1) + ' · <b>' + r.n + ' murid keliru</b></p>' + (r.q ? '<b>' + esc(r.q.prompt) + '</b><p class="ch-muted ch-small">Kunci: ' + esc(r.q.options[r.q.answer]) + '</p>' : '<p class="ch-muted">' + esc(r.id) + '</p>') + (r.tags.length ? '<div class="ch-tags">' + r.tags.map(function (t) { return '<span class="ch-tag is-warn">' + esc(t.label) + ' ×' + t.n + '</span>'; }).join('') + '</div>' : '') + '</li>'; }).join('') + '</ol>' : '<p class="ch-muted">Belum ada bukti per-soal. Muncul otomatis setelah murid menyelesaikan tugas di aplikasi.</p>') + '</section>' +
      (im.misconceptions.length ? '<section class="ch-card ch-card-ink"><p class="ch-kicker">Miskonsepsi dari tugas ini</p><ol class="ch-mis">' + im.misconceptions.slice(0, 4).map(function (m) { return '<li><b>' + esc(m.label) + '</b><small>' + m.n + ' jawaban</small></li>'; }).join('') + '</ol><div class="ch-actions"><button type="button" class="tg-btn is-primary is-small" data-ch="remedial" data-skill="' + esc(a.skills[0]) + '" data-title="' + esc('Remedial: ' + a.title) + '" data-testid="tclass-remedial">' + icon('life-buoy') + ' Buat tugas remedial</button></div></section>' : '') + '</div>';
  }
  function tBraincore(c, env) {
    ...
    return '<div class="ch-body"><section class="ch-card ch-card-ink"><p class="ch-kicker">Prinsip</p><h3>Braincore menyarankan. Guru memutuskan. Murid belajar.</h3><p class="ch-muted">Semua angka di bawah berasal dari bukti murid di kelas ini: laporan sinkron dan soal yang keliru pada tugasmu. Tidak ada AI cloud, tidak ada tebakan tanpa data.</p></section>' +
      ...
      '<section class="ch-card"><p class="ch-kicker">Miskonsepsi terdeteksi</p>' + (fromEvidence.length ? '<p class="ch-muted ch-small">Dari ' + n + ' jawaban keliru pada tugas yang kamu kirim.</p><ol class="ch-mis">' + fromEvidence.map(function (m) { return '<li><b>' + esc(m.label) + '</b><small>' + m.n + '×</small></li>'; }).join('') + '</ol>' : '') + (mis.length ? '<p class="ch-muted ch-small">Dari pola skill kelas:</p><ol class="ch-mis">' + mis.map(function (m) { return '<li><b>' + esc(m.label) + '</b> — ' + esc(m.pattern) + '<small>' + esc(m.lesson) + '</small></li>'; }).join('') + '</ol>' : '') + (!fromEvidence.length && !mis.length ? '<p class="ch-muted">Belum ada bukti cukup. Kirim satu tugas dan tunggu murid mengerjakannya.</p>' : '') + '</section>' +
  ...
  function onTeacherSubmit(e, env) {
    ...
    if (f.getAttribute('data-ch-form') === 'draft-source') {
      if (!d.skills.length) { env.toast('Pilih minimal satu skill.'); return; }
