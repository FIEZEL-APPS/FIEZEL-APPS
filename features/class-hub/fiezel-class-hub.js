...
(function (root) {
  ...
  function tugasView(pend, done) {
    ...
    return '<div class="ch-body"><section><h2 class="ch-h2">Perlu dikerjakan <small>' + pend.length + '</small></h2>' + (pend.length ? pend.map(function (a) { return assignCard(a, true); }).join('') : '<div class="ch-empty" data-testid="class-empty-pending">' + icon('inbox') + (classCode() ? '<p>' + t('kelas.murid-belum-ada-tugas', 'Belum ada tugas baru dari guru. Tugas yang dikirim guru muncul di sini dan di lonceng notifikasi.') + '</p>' : '') + (classCode() ? '' : '<button type="button" class="ch-btn is-primary" data-ch="tab" data-tab="kelas">Masukkan kode ' + WM + '</button>') + '</div>') + '</section>' +
