...
(function (root) {
  ...
  var el = null, env = {}, st = null, ui = { modal: null, drawer: null, filter: '', insightSkill: 'past_tense', attDate: null, pick: {} };
  ...
  function sidebar(c) {
    return '<aside class="tg-side"><div class="tg-brand"><span class="tg-brand-mark">F</span><div><b>FIEZEL</b><small>Ruang Guru</small></div></div>' +
      '<button type="button" class="tg-teacher" data-tg="view" data-view="settings" data-testid="tg-profile">' + icon('user-round') + '<div><b>' + esc(st.teacher.name || 'Guru FIEZEL') + '</b><small>' + esc(st.teacher.school || 'Atur profil →') + '</small></div></button>' +
  ...
  function welcome() {
    return '<section class="tg-welcome" data-testid="tg-welcome"><p class="tg-kicker">Selamat datang</p><h2>Ruang kerja yang membaca kelasmu, lalu memberi tahu <em>siapa yang perlu disapa hari ini</em>.</h2>' +
      ...
      '<div class="tg-welcome-actions"><button type="button" class="tg-btn is-primary is-lg" data-tg="modal" data-kind="new-class" data-testid="tg-welcome-new-class">' + icon('plus') + ' Buat kelas pertama</button><button type="button" class="tg-btn is-ghost is-lg" data-tg="seed-demo" data-testid="tg-welcome-demo">' + icon('sparkles') + ' Coba dengan kelas contoh (18 siswa)</button></div>' +
  ...
  function briefing(c) {
    ...
    var kpi = [['Siswa aktif 7 hari', stt.active7 + '<small>/' + stt.total + '</small>', 'users', stt.total ? stt.active7 / stt.total : 0], ['Rata-rata akurasi', pct(stt.avgAcc), 'target', stt.avgAcc || 0], ['Perlu perhatian', (stt.atRisk + stt.watch) + '<small> siswa</small>', 'alert-triangle', stt.total ? (stt.atRisk + stt.watch) / stt.total : 0], ['Tugas berjalan', stt.openAssignments + '<small> tugas</small>', 'clipboard-list', null]];
  ...
  function classes(c) {
    ...
    return '<div class="tg-toolbar"><div class="tg-tabs">' + st.classes.map(function (k) { return '<button type="button" class="tg-tab' + (k.id === c.id ? ' is-active' : '') + '" data-tg="pick-class" data-id="' + k.id + '">' + esc(k.name) + '<small>' + k.students.length + '</small></button>'; }).join('') + '<button type="button" class="tg-tab is-add" data-tg="modal" data-kind="new-class" data-testid="tg-new-class">+ Kelas</button></div>' +
      '<div class="tg-toolbar-actions"><label class="tg-search">' + icon('search') + '<input type="search" placeholder="Cari siswa…" value="' + esc(ui.filter) + '" data-tg-input="filter" data-testid="tg-student-search"></label><button type="button" class="tg-btn is-ghost" data-tg="modal" data-kind="attendance" data-testid="tg-attendance">' + icon('check-square') + '<span>Absensi</span></button><button type="button" class="tg-btn is-ghost" data-tg="export-csv" data-testid="tg-export-csv">' + icon('download') + '<span>CSV</span></button><button type="button" class="tg-btn is-primary" data-tg="modal" data-kind="add-students" data-testid="tg-add-students">' + icon('user-plus') + '<span>Tambah siswa</span></button></div></div>' +
      '<section class="tg-card tg-class-meta"><div><p class="tg-kicker">' + esc(c.subject || 'English') + ' · Level ' + esc(c.level) + (c.demo ? ' · <span class="tg-demo">data contoh</span>' : '') + '</p><h3>' + esc(c.name) + '</h3><small>Kode kelas <b class="tg-mono">' + esc(c.code) + '</b> — murid mengetiknya saat onboarding; hasil latihannya masuk ke sini otomatis.</small></div><div class="tg-actions"><button type="button" class="tg-btn is-ghost is-small" data-tg="modal" data-kind="edit-class">' + icon('pencil') + ' Ubah</button><button type="button" class="tg-btn is-danger is-small" data-tg="delete-class" data-testid="tg-delete-class">' + icon('trash-2') + ' Hapus kelas</button></div></section>' +
      (list.length ? '<div class="tg-table-wrap"><table class="tg-table" data-testid="tg-student-table"><thead><tr><th>Siswa</th><th>Status</th><th>Akurasi</th><th>Terakhir aktif</th><th>Kehadiran</th><th>Tugas</th><th></th></tr></thead><tbody>' +
        ...
        '<section class="tg-card tg-center"><h3>Belum ada siswa di kelas ini</h3><p class="tg-muted">Tambah nama siswa (bisa tempel dari daftar absen), atau bagikan kode kelas <b>' + esc(c.code) + '</b> supaya hasil latihan murid masuk sendiri.</p><button type="button" class="tg-btn is-primary" data-tg="modal" data-kind="add-students">' + icon('user-plus') + ' Tambah siswa</button></section>');
  }
  ...
  function insights(c) {
    ...
    if (!c.students.length) return '<section class="tg-card tg-center"><h3>Belum ada data untuk dianalisis</h3><p class="tg-muted">Tambah siswa atau tempel kode hasil latihan murid dulu.</p></section>';
  ...
  function drawer(c) {
    ...
    return '<div class="tg-scrim" data-tg="close"></div><aside class="tg-drawer" role="dialog" aria-label="Detail siswa" data-testid="tg-drawer">' +
      '<div class="tg-drawer-head">' + avatar(s, 'lg') + '<div><h3>' + esc(s.name) + '</h3><p class="tg-muted">' + riskPill(r) + ' · ' + (r.inactiveDays == null ? 'belum aktif' : r.inactiveDays === 0 ? 'aktif hari ini' : r.inactiveDays + ' hari tidak belajar') + '</p></div><button type="button" class="tg-icon-btn" data-tg="close" aria-label="Tutup">' + icon('x') + '</button></div>' +
      '<div class="tg-drawer-body">' +
      '<div class="tg-action-box"><p class="tg-kicker">Tindakan yang disarankan</p><p>' + esc(r.action) + '</p><div class="tg-actions"><button type="button" class="tg-btn is-primary is-small" data-tg="modal" data-kind="greet" data-id="' + s.id + '">' + icon('message-circle-heart') + ' Kartu sapa</button><button type="button" class="tg-btn is-ghost is-small" data-tg="modal" data-kind="parent" data-id="' + s.id + '">' + icon('file-text') + ' Laporan ortu</button><button type="button" class="tg-btn is-ghost is-small" data-tg="modal" data-kind="assign" data-target="' + s.id + '"' + (r.weak ? ' data-skill="' + r.weak.skill + '"' : '') + '>' + icon('plus') + ' Tugas khusus</button></div></div>' +
      ...
      '<h4>Tugas</h4>' + (pend.length ? '<ul class="tg-mini-list">' + pend.map(function (p) { return '<li>' + esc(p.a.title) + (p.late ? ' <strong class="tg-late">lewat</strong>' : '') + ' <button type="button" class="tg-link" data-tg="mark-done" data-id="' + p.a.id + '" data-sid="' + s.id + '">tandai selesai</button></li>'; }).join('') + '</ul>' : '<p class="tg-muted">Semua tugas selesai.</p>') +
      '<h4>Kontak orang tua</h4><form data-tg-form="phone" data-id="' + s.id + '" class="tg-inline"><input name="phone" inputmode="tel" value="' + esc(s.parentPhone) + '" placeholder="08xx / 62xx" data-testid="tg-phone-input"><button type="submit" class="tg-btn is-small is-ghost" data-testid="tg-phone-save">Simpan</button></form>' +
      '<h4>Catatan guru</h4><form data-tg-form="note" data-id="' + s.id + '" class="tg-inline"><input name="text" required placeholder="Catatan singkat…" data-testid="tg-note-input"><button type="submit" class="tg-btn is-small is-ghost" data-testid="tg-note-save">Tambah</button></form>' +
      ...
      '<div class="tg-actions tg-drawer-foot"><button type="button" class="tg-btn is-danger is-small" data-tg="delete-student" data-id="' + s.id + '">' + icon('user-minus') + ' Hapus dari kelas</button></div></div></aside>';
  }
  ...
  function modal(c) {
    ...
    if (m.kind === 'new-class' || m.kind === 'edit-class') {
      ...
      body = '<p class="tg-muted">' + (m.kind === 'greet' ? 'Disusun dari data ' + s2.name + '. Ubah seperlunya agar terdengar seperti kamu.' : 'Angka diambil langsung dari data. Tanda tangan memakai profil gurumu.') + '</p><textarea class="tg-code is-text" rows="' + (m.kind === 'greet' ? 5 : 12) + '" data-tg-input="draft" data-testid="tg-draft">' + esc(m.draft != null ? m.draft : text) + '</textarea>' +
  ...
  function board(c) {
    ...
    return '<div class="tg-board" data-testid="tg-board"><button type="button" class="tg-board-close" data-tg="close" aria-label="Tutup mode papan">' + icon('x') + ' Tutup</button><p class="tg-board-kicker">' + esc(c.name) + ' · ' + esc(new Date().toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long' })) + '</p><h2>Kemajuan kita minggu ini</h2>' +
  ...
  function onClick(e) {
    ...
    switch (act) {
      ...
      case 'seed-demo': { var d = T.seedDemo(); st.classes.push(d); st.activeClassId = d.id; st.onboarded = true; ui.modal = null; st.view = 'briefing'; toast('Kelas contoh dimuat — 18 siswa, 2 tugas, data 14 hari.'); break; }
      ...
      case 'delete-assign': if (!c || !confirm('Hapus tugas ini?')) return; c.assignments = c.assignments.filter(function (a) { return a.id !== id; }); break;
  ...
  function onSubmit(e) {
    ...
    if (kind === 'new-class') { var k = T.newClass(fd.get('name'), fd.get('level'), fd.get('subject')); st.classes.push(k); st.activeClassId = k.id; st.onboarded = true; ui.modal = null; st.view = 'classes'; toast('Kelas ' + k.name + ' dibuat. Kode: ' + k.code); }
    else if (kind === 'edit-class' && c) { c.name = String(fd.get('name')).slice(0, 60); c.level = fd.get('level'); c.subject = fd.get('subject'); ui.modal = null; }
    ...
    else if (kind === 'assign' && c) { var skills = fd.getAll('skills').slice(0, 3), targets = fd.getAll('targets'); if (!skills.length) { toast('Pilih minimal satu skill.'); return; } var a = T.buildAssignment({ title: fd.get('title'), skills: skills, count: fd.get('count'), deadline: fd.get('deadline'), mode: fd.get('mode'), targets: targets, avoid: c.sentItemIds }); c.assignments.push(a); c.sentItemIds = (c.sentItemIds || []).concat(a.itemIds).slice(-120); st.view = 'assignments'; ui.modal = { kind: 'share-assign', id: a.id }; ui.drawer = null; saveMinutes(25); toast('Tugas tersusun: ' + a.itemIds.length + ' soal dari bank FIEZEL.'); }
