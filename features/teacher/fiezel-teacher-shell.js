  ...
  function modal(c) {
    ...
    if (m.kind === 'new-class' || m.kind === 'edit-class') {
      ...
      body = '<form data-tg-form="assign" class="tg-form">' + tabHeader + tabContent +
        '<label class="tg-label">' + t('guru.judul-tugas-bab', 'Judul Tugas / Bab') + '<input name="title" maxlength="80" value="' + esc(curUnit && tab === 'curriculum' ? curUnit.title : '') + '" placeholder="' + (curUnit && tab === 'curriculum' ? esc(curUnit.title) : 'Kosongkan untuk judul otomatis') + '" data-testid="tg-assign-title"></label>' +
