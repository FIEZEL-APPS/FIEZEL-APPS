...
(function(root,factory){
  ...
  class TTSService{
    ...
    play(text,options={}){if(!capabilities().tts)return Promise.reject(new Error('tts_unavailable'));this.stop();return new Promise((resolve,reject)=>{try{const u=new global.SpeechSynthesisUtterance(String(text||''));u.lang=this.config.language;u.rate=clamp(options.rate??this.config.ttsRate,.55,1.3);u.onend=()=>resolve({provider:'browser-speech-synthesis'});u.onerror=()=>reject(new Error('tts_failed'));global.speechSynthesis.speak(u)}catch(e){reject(e)}})}
  }
