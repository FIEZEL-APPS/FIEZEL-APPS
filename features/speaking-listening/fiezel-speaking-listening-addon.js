...
(function(root,factory){
  ...
  class Controller{
    ...
    finishItem(item,result,prefix=''){
      const ms=now()-this.startedAt;this.store.record(this.domain,item,result,ms);this.emitEvidence();const label=tr(result.passed?'sl.passed':'sl.notPassed');const note=this.domain==='speaking'?tr('sl.speakingScore',{score:result.score,metric:result.metric.replace(/_/g,' ')}):tr('sl.score',{score:result.score});
      const fb=this.root.querySelector('[data-feedback]');if(fb)fb.innerHTML=`${prefix}<div class="fsl-feedback"><strong data-testid="skills-lab-result">${esc(label)}</strong><span>${esc(note)}</span>${this.domain==='listening'?`<p><b>${esc(tr('sl.script'))}</b> ${esc(item.script)}</p>`:`<p><b>${esc(tr('sl.sampleAnswer'))}</b> ${esc(item.sampleAnswer||item.targetText||'')}</p>`}<div class="fsl-actions"><button class="fsl-primary" data-next data-testid="skills-lab-next">${esc(tr('sl.next'))}</button></div></div>`;
