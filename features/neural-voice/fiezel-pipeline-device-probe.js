(function(root,factory){
  ...
  const PROBE_SENTENCES=Object.freeze([
    'Learning English becomes easier when practice is regular, focused, and connected to real goals. A student can build confidence by reading useful passages, noticing new vocabulary, checking how grammar works inside complete sentences, and speaking aloud even when every word is not perfect. Small sessions repeated across the week are often more valuable than one long session followed by several days without practice, because memory improves when ideas are recalled again before they are completely forgotten.',
    'For this diagnostic test, FIEZEL is intentionally reading a longer passage so the local neural voice must process more than one chunk. The first part should begin playing normally, and while that audio is still playing the application may prepare the next chunk in the background without starting a second model inference at the same time. The second part must wait for the first playback to finish, then continue in order without repeating words, skipping text, switching to browser speech, or producing overlapping audio.'
  ]);
  ...
  async function run(env){
    ...
    if(status&&status.prepared===true&&status.ready!==true&&typeof runtime.ensureReady==='function')await runtime.ensureReady();
