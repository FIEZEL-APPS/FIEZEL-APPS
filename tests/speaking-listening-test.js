...
/* 2026-08-30: akhir baris dinormalkan sebelum di-hash. Di checkout Windows dengan core.autocrlf
   berkas bank di disk ber-CRLF, sementara tools/dev/rebuild-speaking-listening-data.js selalu menulis LF -
...
test('data rebuild is idempotent',()=>{const files=[path.join(feature,'listening-bank-v1.json'),path.join(feature,'speaking-bank-v1.json')],before=files.map(hash);child.execFileSync(process.execPath,[path.join(root,'tools/dev/rebuild-speaking-listening-data.js')],{stdio:'ignore'});assert.deepStrictEqual(files.map(hash),before)});
