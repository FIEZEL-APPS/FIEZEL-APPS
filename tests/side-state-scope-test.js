#!/usr/bin/env node
/**
 * GERBANG RUANG-AKUN SIDE-STATE (tests/side-state-scope-test.js)
 *
 * LUBANG YANG DITUTUP
 * -------------------
...
test('gate ini terdaftar di CI', () => {
  ...
  assert.ok(workflow.includes('node tests/side-state-scope-test.js'), 'gate yang tidak berjalan di CI bukan gate');
});
