#!/usr/bin/env node
/**
 * PHASE 0 · tests/id-golden-snapshot-test.js — GERBANG BASELINE EMAS BAHASA INDONESIA
 *
 ...
 *
 * DUA MODE:
 *   node tests/id-golden-snapshot-test.js                  → verifikasi terhadap baseline (CI)
 *   node tests/id-golden-snapshot-test.js --write-baseline → tulis ulang baseline (TINDAKAN SADAR,
 *                                                      hanya ketika perubahan naskah memang
 *                                                      disengaja dan sudah direview owner)
 *
 * ENV:
 *   FIEZEL_ROOT     → root repo FIEZEL (default: __fzRoot, sesuai konvensi gerbang lain)
 ...
 * terdaftar oleh tests/gate-registry-test.js begitu berkas ini mendarat di root repo.
 */
'use strict';
...
const BASELINE_PATH = process.env.FIEZEL_BASELINE || path.join(__fzRoot, 'id-golden-baseline.json');
