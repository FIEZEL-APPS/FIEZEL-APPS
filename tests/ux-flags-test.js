...
/* -- T4: nama tak dikenal gagal ke arah AMAN (false), bukan undefined ------------------ */
{
  ...
  check(/return resolved\[flag\] === true;/.test(fs.readFileSync(path.join(__fzRoot, 'fiezel-ux-flags.js'), 'utf8')),
...
/* -- T6: app.js benar-benar MEMAKAI benderanya --------------------------------------- */
{
  ...
  const readers = [
    ...
    ['placementLite', APP],
    ['sessionSummary', APP],
    ['leanIntro', fs.readFileSync(path.join(__fzRoot, 'features/onboarding/fiezel-onboarding.js'), 'utf8')]
  ];
