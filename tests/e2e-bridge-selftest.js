// tests/e2e-bridge-selftest.js — pembuktian bahwa `tools/fiezel-e2e-bridge.mjs` bisa MERAH.
//
// ==========================================================================================
...
// `tests/no-network-test.js` dengan alasan yang diperiksa, bukan dengan pelonggaran pemindai.
//
...
const GATE = path.join(__fzRoot, 'tools', 'fiezel-e2e-bridge.mjs');
...
(function () {
  ...
  function boot() {
    // URL disusun dari konstanta, bukan literal di dalam fetch(): pemindai tests/no-network-test.js
...
(async function main() {
  ...
  // Probe TANPA FIEZEL_E2E_REPORT: begitulah tests/no-network-test.js menjalankannya, dan pada jalur
  ...
  check('SKIP tanpa FIEZEL_E2E_REPORT tetap exit 0 dan tidak menulis artefak ke akar repo',
    skipTanpaReport.code === 0 && !fs.existsSync(path.join(__fzRoot, 'E2E-BRIDGE-REPORT.json')),
  ...
  check('Laporan gerbang tidak pernah ditulis ke akar repo (working tree bersih)',
    !fs.existsSync(path.join(__fzRoot, 'E2E-BRIDGE-REPORT.json')), 'E2E-BRIDGE-REPORT.json tidak ada di akar');
