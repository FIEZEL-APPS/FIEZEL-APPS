// tests/staging-live-test.js — gerbang runtime NYATA untuk Worker `fiezel-api-staging`.
//
// ==========================================================================
// KENAPA BERKAS INI ADA
// ==========================================================================
// `tests/cf-live-contract-test.js` sudah menembak Worker hidup, tetapi ia dengan
...
// push menembak Worker sungguhan. `tests/no-network-test.js` ikut memeriksa larangan
...
const reportPath = process.env[ENV_REPORT]
  ...
  : path.join(__fzRoot, 'STAGING-LIVE-REPORT.json');
...
if (!rawBase || !edgeSecret) {
  ...
  console.log([
    ...
    '  ' + ENV_EDGE + '="$(cat /jalur/rahasia)" node tests/staging-live-test.js --report'
  ].join('\n'));
  writeReport({
...
async function run() {
  ...
  const ttsSource = fs.readFileSync(path.join(__fzRoot, 'workers/api/tts/route-tts.js'), 'utf8');
  ...
  const wrangler = fs.readFileSync(path.join(__fzRoot, 'workers/api/wrangler.toml'), 'utf8');
  ...
  notes.push('CRON — belum terbukti di runtime: eksekusi `scheduled()` itu sendiri. '
    ...
    + 'tetap dibuktikan di atas stub oleh tests/cf-wiring-test.js.');
