...
//
// Berkas ini node murni dan NOL JARINGAN. Ia melengkapi `tests/prerender-dryrun-test.js` (yang menjaga
...
// HANYA `fetch` yang dijerat di sini, dan itu keputusan sadar, bukan kelalaian: `tests/no-network-test.js`
// menolak gerbang non-allowlist yang me-`require` modul socket (http/https/net/dns) dan menjaga
// daftar "jerat saja"-nya tetap SATU nama (`tests/prerender-dryrun-test.js`, yang sudah memasang jerat
...
(async () => {
  ...
  check('Gerbang rencana ini dijalankan di workflow pra-render sebelum apply',
    wf.includes('tests/prerender-plan-test.js') &&
    wf.indexOf('tests/prerender-plan-test.js') < wf.indexOf('name: Produksi'), 'gerbang dulu');
  const quality = fs.readFileSync(path.join(root, '.github/workflows/quality.yml'), 'utf8');
  check('Gerbang ini terdaftar di quality.yml', /node tests\/prerender-plan-test\.js/.test(quality), 'terdaftar');
