#!/usr/bin/env node
/**
 * GERBANG RESET SIDE-STATE (tests/reset-side-state-test.js)
 *
...
test('gate ini terdaftar di CI', () => {
  ...
  assert.ok(workflow.includes('node tests/reset-side-state-test.js'), 'gate yang tidak berjalan di CI bukan gate');
});
