#!/usr/bin/env node
/**
 * GERBANG META PENDAFTARAN (tests/gate-registry-test.js)
 *
 * ==========================================================================
 ...
 * `tests/bank-soal-audit-test.js` dan `tests/app-report-control-path-test.js`. Keduanya
 ...
 * `tests/cf-live-contract-test.js` dan `tests/staging-live-test.js` tanpa meneruskan env
...
const EXCLUSIONS = new Map([
  ...
  ['audit/bank-audit.js', {
    ...
    reason:
      ...
      'bank soal dijaga oleh tests/bank-soal-audit-test.js (terdaftar) dan grammar-quality-audit.js ' +
      '(terdaftar). Cara menjalankan alat ini: `node audit/bank-audit.js` (lihat ' +
      'docs/handoffs/FIEZEL-M025125-SIDAK-BANK-SOAL-HANDOFF.md).'
  }],
...
// nama gerbang di dalam prosa (mis. "`tests/e2e-bridge-selftest.js` membuktikan …"), dan menghitung
...
check(
  ...
  unregistered.length === 0
    ...
    : `tidak terdaftar dan tidak dikecualikan: ${unregistered.join(', ')} — daftarkan di ` +
      `${WORKFLOW}, atau tambahkan entri EXCLUSIONS di tests/gate-registry-test.js dengan alasan tertulis`
);
 ...
 * URL — nol paket keluar, nol DNS. `tests/no-network-test.js` tetap penjaga utama larangan
...
for (const [file, meta] of LIVE_GATES) {
  ...
  // tests/staging-live-test.js butuh dua env; isi keduanya supaya ia melewati cabang SKIP dan
  // benar-benar sampai ke validasi base URL.
  if (file === 'tests/staging-live-test.js') env.FIEZEL_STAGING_EDGE = 'x-uji-lokal-bukan-rahasia-sungguhan';
 ...
 */
check('gerbang-meta-ini-terdaftar', invoked.has('tests/gate-registry-test.js'), `${WORKFLOW} memanggil tests/gate-registry-test.js`);
