// tests/owner-dashboard-test.js — gerbang untuk workers/owner (dashboard owner, E6).
//
...
// tests/d1-schema-contract-test.js langsung terhadap DDL migrasi.
...
assert(!fs.existsSync(path.join(__fzRoot, 'owner.html')), 'owner.html tidak boleh ada di repo (sw.js precache)');
const swSource = fs.readFileSync(path.join(__fzRoot, 'sw.js'), 'utf8');
...
// tests/d1-schema-contract-test.js, jadi fixture ini tidak bisa mengarang metrik yang tidak ada.
...
(async () => {
  ...
  // workers/api/ oleh tests/d1-schema-contract-test.js. Tanpa dua rantai ini, gerbang bisa hijau di
