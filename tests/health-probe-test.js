/**
 * tests/health-probe-test.js — GERBANG untuk `tools/fiezel-health-probe.mjs`.
 *
...
// Buang komentar sebelum memindai kode (pola tests/audio-asset-pipeline-test.js:290-292),
 ...
 * ===================================================================================== */
check('Gerbang ini terdaftar di quality.yml', workflow.includes('node tests/health-probe-test.js'), 'quality.yml');
 ...
 * ===================================================================================== */
// `tests/no-network-test.js` hanya memindai berkas `*-test.js` / `*-audit.js` / `*-selftest.js`
// di akar repo dan di `tests/` (m025-254: seluruh gerbang pindah ke sana), jadi
// `tools/fiezel-health-probe.mjs` tidak pernah masuk pemindaiannya —
...
const noNetwork = mustRead('tests/no-network-test.js');
check('Pemindai no-network memang tidak mencakup tools/ (jadi §4+§9 yang menjaga, dan itu dicatat)',
  /gatesIn\('tests'\)/.test(noNetwork) && /-\(test\|audit\|selftest\)\\\.js\$/.test(noNetwork)
  && !/gatesIn\('tools/.test(noNetwork),
  'cakupan: berkas gerbang di akar repo + tests/, bukan tools/');
