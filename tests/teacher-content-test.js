/**
 * tests/teacher-content-test.js — GERBANG model konten guru (§3-§6, §13, §14, §18).
 *
...
(async () => {
  const C = await import('file://' + path.join(__fzRoot, 'workers/api/teacher/content-core.js'));
  ...
  const journey = fs.readFileSync(path.join(__fzRoot, 'features/personal-journey/fiezel-personal-journey.js'), 'utf8');
  const coreBrain = fs.readFileSync(path.join(__fzRoot, 'features/brain/fiezel-core-brain.js'), 'utf8');
