/**
 * tests/teacher-braincore-test.js — GERBANG integrasi konten guru <-> Braincore (§15-§17, §20, §29).
 *
...
function loadCoreBrainModule(file, globalName) {
  const code = fs.readFileSync(path.join(__fzRoot, file), 'utf8');
...
(async () => {
  const B = await import('file://' + path.join(__fzRoot, 'workers/api/teacher/braincore-bridge.js'));
