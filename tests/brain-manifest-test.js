 ...
 *   - klaim otoritas kunci harus sesuai wiring nyata (memory aktif, BKT aktif sejak m025-331);
...
const BRAIN_DIR = path.join(__fzRoot, 'features', 'brain');
...
test('bundleVersion 3.9.0 (bktUnlock shadow -> active, m025-331)', () => {
...
test('minAppVersion sama dengan FIEZEL_VERSION di version.js (dibaca, bukan dikarang)', () => {
  const src = fs.readFileSync(path.join(__fzRoot, 'version.js'), 'utf8');
...
test('klaim otoritas kunci: memory aktif, bktUnlock aktif sejak m025-331', () => {
  // Sampai m025-331 baris kedua berbunyi 'shadow', sesuai temuan council waktu itu.
...
test('contentCompatibility cocok dengan deklarasi schemaVersion grammar-templates.json', () => {
  const raw = JSON.parse(fs.readFileSync(path.join(__fzRoot, 'grammar-templates.json'), 'utf8'));
