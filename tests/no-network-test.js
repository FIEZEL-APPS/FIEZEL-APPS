// tests/no-network-test.js — gerbang mutu tidak boleh bergantung pada jaringan sungguhan.
//
...
//
//   tests/regression-test.js:70   const fetch=async url=>{ … fs.existsSync(file) … }   ← definisi mock
//   tests/regression-test.js:77   const ctx={…,fetch,…}                                ← memakai mock itu
//
// Klaim cf-a9 (temuan #5 / R3) bahwa `tests/regression-test.js:77` membocorkan `fetch` asli
...
// sendiri, bukan didaftar putih): tests/regression-test.js, tests/ai-integration-test.js,
// tests/lesson-experience-test.js, tests/notification-reminder-test.js, grammar-quality-audit.js,
// tests/adaptive-policy-test.js, content-audit.js, tests/learner-evidence-test.js,
// tests/alrs-behavior-test.js, tests/policy-outcome-test.js.
//
...
//   tests/http-smoke-test.js        - menyalakan server loopback untuk memeriksa aset HTTP nyata
//   tests/e2e-level-grammar-test.js - :12 require('http'), :37 server.listen(0,'127.0.0.1'),
//                               :82 WebSocket ke CDP Chromium lokal
//   tests/cf-live-selftest.js       - :41 require('node:http'), server.listen(0,'127.0.0.1'):
//                               ia menyalakan Worker `fiezel-api` TIRUAN untuk membuktikan
//                               `tests/cf-live-contract-test.js` benar-benar bisa merah (21
...
//                               tanpa keluar mesin.
//   tests/e2e-bridge-selftest.js    - :require('node:https'), server.listen(0,'127.0.0.1'):
...
// `tests/cf-live-selftest.js` lolos hanya karena namanya tidak berakhiran `-test.js` — dan
// "lolos karena nama berkas" adalah kebalikan dari daftar yang disengaja.
//   tests/ui-render-audit-test.js  - require('http') untuk MENYAJIKAN repo ini sendiri di
...
const SOCKET_ALLOWLIST = new Set(['tests/http-smoke-test.js', 'tests/e2e-level-grammar-test.js', 'tests/cf-live-selftest.js', 'tests/e2e-bridge-selftest.js', 'tests/ui-render-audit-test.js']);

// Kelas kedua yang berbeda secara MAKNA, bukan sekadar pengecualian kedua:
//   tests/prerender-dryrun-test.js - :33-38 require('https')/require('http') SEMATA-MATA untuk
...
//   tests/cf-live-contract-test.js - satu-satunya gerbang yang SENGAJA menembak host
...
//   tests/staging-live-test.js      - anggota KEDUA kelas ini, dan ia menembak lebih jauh:
...
// Env-nya PER BERKAS, bukan satu konstanta global: `tests/cf-live-contract-test.js` dan
// `tests/staging-live-test.js` menembak dua lingkungan berbeda, dan menyamakan nama env
// keduanya akan membuat satu variabel menyalakan gerbang yang tidak dimaksud.
const ENV_GATED_LIVE_ALLOWLIST = new Map([
  ['tests/cf-live-contract-test.js', 'FIEZEL_CF_LIVE_BASE'],
  ['tests/staging-live-test.js', 'FIEZEL_STAGING_BASE']
]);
...
// Buang komentar SEBELUM memindai — pola yang sudah dipakai tests/audio-asset-pipeline-test.js:290-292
...
function analyzeSource(file, rawCode, { allowSocket = false, trapOnly = false } = {}) {
  ...
  for (const injection of injections) {
    ...
    //     Sebelas gerbang existing memakai bentuk ini (tests/runtime-stage8-test.js:7,
    //     tests/puter-auth-coop-test.js:24, tests/backup-ui-test.js:37 lewat pengenal, dst).
    ...
    //     (pola `fetchMock` di tests/observability-privacy-test.js:67-69).
    ...
    //     mendahului secara POSISI (pola tests/regression-test.js:70 → :77).
...
check('Detektor MEMBEBASKAN mock lokal yang mendahului (pola tests/regression-test.js:70-77)', kinds(FIXTURES.amanDenganMockLokal) === '', kinds(FIXTURES.amanDenganMockLokal) || '(kosong)');
...
check('Detektor MEMBEBASKAN mock inline `fetch: async url => …` (pola tests/runtime-stage8-test.js:7)', kinds(FIXTURES.amanMockInline) === '', kinds(FIXTURES.amanMockInline) || '(kosong)');
check('Detektor MEMBEBASKAN mock bernama lain yang didefinisikan lebih dulu (pola tests/observability-privacy-test.js:67)', kinds(FIXTURES.amanMockBernamaLain) === '', kinds(FIXTURES.amanMockBernamaLain) || '(kosong)');
...
/* m025-254: seluruh gerbang pindah dari akar repo ke tests/. Pemindaian yang hanya membaca
   akar akan tetap "hijau" - bukan karena bersih, melainkan karena tidak menemukan apa pun,
   tepat kegagalan yang dijaga assert `files.length >= 100` di bawah. Ia kini menyisir akar
   DAN tests/, dan menyebut setiap berkas dengan path relatif repo, bentuk yang sama dengan
...
// `tests/e2e-bridge-selftest.js` (paket A5), dan `tests/ui-render-audit-test.js` (audit UI/UX
...
check('Pemindaian mencakup berkas *-selftest.js (bukan hanya *-test.js/*-audit.js)',
  files.includes('tests/cf-live-selftest.js') && files.includes('tests/e2e-bridge-selftest.js'),
...
for (const [file, envVar] of ENV_GATED_LIVE_ALLOWLIST) {
  ...
  // bukan dengan membaca janji di komentarnya. `tests/cf-live-selftest.js` menguji hal yang
...
const E2E_SELFTEST = 'tests/e2e-bridge-selftest.js';
const e2eProblems = [];
{
...
check('Gerbang ini terdaftar di quality.yml', workflow.includes('node tests/no-network-test.js'), 'quality.yml');
...
check('Gerbang live dan self-test-nya terdaftar di quality.yml',
  workflow.includes('node tests/cf-live-contract-test.js') && workflow.includes('node tests/cf-live-selftest.js'),
  'quality.yml');
...
// `if:` itu. `tests/workflow-actor-gate-test.js` cek (H) memeriksa hal yang sama dari sisi lain,
...
// dicetak, gerbang live tidak dihitung sebagai bukti) dijaga `tests/gate-registry-test.js`.
...
check('Gerbang staging live terdaftar di quality.yml', workflow.includes('node tests/staging-live-test.js'), 'quality.yml');
