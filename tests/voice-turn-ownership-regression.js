'use strict';
const __fzRoot = require('path').join(__dirname, '..'); /* m025-254: berkas ini pindah dari akar ke tests/ bersama gerbang lain; __dirname dulu BERARTI akar repo. */
...
async function testVoiceSayOwnershipAndStop() {
  const voicePath = path.resolve(__fzRoot, 'features/neural-voice/fiezel-voice-say.js');
...
function testBridgeRejectsStaleEvents() {
  const bridgePath = path.resolve(__fzRoot, 'features/neural-voice/fiezel-speech-bridge.js');
