#!/usr/bin/env node
/**
 * GERBANG LAPORAN RANTAI KONTEN (tests/content-chain-report-test.js)
 *
 * KLAIM YANG DIUJI DI SINI, DAN KENAPA IA BERBEDA DARI tests/content-chain-test.js
 * -------------------------------------------------------------------------
 * `tests/content-chain-test.js` menguji LOGIKA rantai atas fixture. Gerbang ini menguji bahwa
...
test('gate ini terdaftar di CI', () => {
  ...
  assert.ok(workflow.includes('node tests/content-chain-report-test.js'), 'gate yang tidak berjalan di CI bukan gate');
});
