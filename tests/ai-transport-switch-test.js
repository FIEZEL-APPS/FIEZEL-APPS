// tests/ai-transport-switch-test.js — gerbang PERILAKU untuk sakelar transport permintaan AI.
//
...
// (`fetchMock`), pola yang dikenali tests/no-network-test.js.
'use strict';
...
check('Gerbang ini terdaftar di quality.yml',
  workflow.includes('node tests/ai-transport-switch-test.js'), 'quality.yml');
...
function makeSandbox({ aiMode = 'off', enabled = true, respond, puterExec, online = true } = {}) {
  ...
  const sandbox = {
    ...
    closeModal: () => {},
    // Pelolosan markdown punya gerbangnya sendiri (tests/ai-integration-test.js). Di sini cukup
