...
/* tests/coordination-guard-test.js — PENEGAK protokol koordinasi.
 *
 * KENAPA GERBANG INI ADA. `docs/AGENTS-COORDINATION.md` sudah melarang tabrakan sejak v1.2, dan
...
for (const f of dilacak) {
  ...
  if (f === 'tests/coordination-guard-test.js') continue;
  let isi;
  try {
...
/* ---------------------------------------------------------------- (F) siaran ------------- */
const SIARAN = 'docs/MASTER-BROADCAST.md';
...
if (gagal) {
  console.error('coordination-guard-test GAGAL — baca docs/MASTER-BROADCAST.md, khususnya prosedur P10.');
  process.exit(1);
}
