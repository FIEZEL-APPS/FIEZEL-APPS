...
async function babWorker() {
  ...
  const unknownSub = await app.call('GET', '/api/owner/learner-evidence?sub=character-system-6', { headers: ownerHeaders });
...
async function babDashboard() {
  ...
  check('(E) HTML dashboard memuat tautan pilih-murid ber-sub', /learner=character-system-6/.test(html));
