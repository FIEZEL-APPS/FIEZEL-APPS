...
async function babWorker() {
  ...
  const unknownSub = await app.call('GET', '/api/owner/learner-evidence?sub=curriculum-engine-6', { headers: ownerHeaders });
...
function babKlien() {
  ...
  /* m025-254: jendela jarak dinaikkan 4000 -> 5000. Bukan pelonggaran janji: yang dijaga
     tetap "purge antrean lane ada DI DALAM resetProgress()". Pemindahan gerbang ke tests/
     menambah 6 karakter pada tiap sebutan nama berkas uji di komentar app.js, dan sebelas
     sebutan di dalam fungsi ini sudah cukup mendorong panggilannya melewati batas lama -
     angka 4000 memang jarak, bukan kontrak. */
  check('(F) klien: reset progres ikut mengosongkan antrean lane per-murid',
    /function resetProgress\(\)[\s\S]{0,5000}identityEvidenceQueue\(\)[\s\S]{0,200}q\.purge\(\)/.test(app));
...
async function babDashboard() {
  ...
  check('(E) HTML dashboard memuat tautan pilih-murid ber-sub', /learner=curriculum-engine-6/.test(html));
...
function babDokumen() {
  ...
  const privacy = read('docs/BRAIN-DATA-PRIVACY.md');
  check('(F) docs/BRAIN-DATA-PRIVACY.md mendokumentasikan lane per-murid',
    /learner_evidence/.test(privacy) && /180/.test(privacy));
  check('(K) docs/BRAIN-DATA-PRIVACY.md mendokumentasikan nama learner di server',
    /learner_name/.test(privacy));
  const workflow = read('.github/workflows/quality.yml');
  check('gerbang ini dipanggil quality.yml',
    /node tests\/braincore-learner-identity-test\.js/.test(workflow));
}
