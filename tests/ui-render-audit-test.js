 ...
 * semuanya membaca TEKS SUMBER dengan regex: `tests/contrast-test.js` mencocokkan pola warna di
 * style.css, `tests/a11y-test.js` mencari literal di app.js, `tests/ui-structure-test.js` menghitung
 ...
 * dan DOM disusun:
 *
 *   - tests/contrast-test.js:353 SUDAH menguji `.section-head h1`, tetapi ia MEMODELKAN warnanya
...
function seedScript() {
  return () => {
    try {
      ...
      localStorage.setItem('fiezel-auth-v1', JSON.stringify({ v: 1, signedIn: true, at: Date.now(), role: 'murid', via: 'akun' })); // m025-367: lewati layar masuk wajib
...
async function main() {
  ...
  try {
     ...
     * bawah 20 pasangan kalibrasi (dan tests/olm-test.js:91 mengunci token itu sebagai kontrak
