...
(async()=>{
  ...
  const files=['grammar-templates.json','vocabulary-master.json','reading-bank.json'],hash=f=>crypto.createHash('sha256').update(fs.readFileSync(path.join(__fzRoot,f))).digest('hex'),before=Object.fromEntries(files.map(f=>[f,hash(f)]));
  ...
  fs.writeFileSync(path.join(__fzRoot,'CONTENT-PROMOTION-PROOF.json'),JSON.stringify(proof,null,2)+'\n');
