...
test('Background Fetch mengambil alih, dan jalur potongan berhenti saat itu', () => {
  const SRC = require('fs').readFileSync(require('path').join(__fzRoot, 'features/neural-voice/fiezel-voice-offline-autoload.js'), 'utf8');
...
test('mesin cadangan bukan kokoro, dan alasannya tertulis', () => {
  ...
  if (fs.existsSync(path.join(__fzRoot, 'vendor/kokoro-model'))) {
