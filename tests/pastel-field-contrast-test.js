 ...
 *
 * tests/contrast-test.js yang sudah ada memeriksa PASANGAN WARNA yang sudah diketahui. Ia tidak
...
test('palet lama tidak tertinggal di berkas mana pun', () => {
  ...
  for (const file of scan) {
    let text;
    try { text = fs.readFileSync(path.join(__fzRoot, file), 'utf8'); } catch { continue; }
...
const THEME_JS = fs.readFileSync(path.join(__fzRoot, 'features/ui/fiezel-ui-manager.js'), 'utf8');
