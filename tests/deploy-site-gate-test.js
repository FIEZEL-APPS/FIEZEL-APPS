...
/* tests/deploy-site-gate-test.js — PENEGAK keselamatan unggahan situs produksi.
 *
 ...
 *
 *  (C) PENJAGA AKTOR. Workflow ini memegang kredensial hosting. `tests/workflow-actor-gate-test.js`
...
check('B berkas gerbang uji MEMANG terbuang (kalau tidak, ratusan berkas CI ikut ke server murid)',
  polaKecuali.some((p) => dibuang(p, 'tests/deploy-site-gate-test.js')) &&
...
check('C penjaga aktor owner terpasang tingkat job',
  /github\.actor\s*==\s*'FIEZEL-APPS'/.test(wf), 'sinkron dengan tests/workflow-actor-gate-test.js');
