...
- Guardrail tests that must keep passing after implementation: `tests/paw-mascot-test.js` contracts, `tests/onboarding-test.js`, plus a new test asserting `react()` warns on unknown events **and** that `app.js` contains no call to an event outside the documented vocabulary (regression net for defect-1-class bugs).
