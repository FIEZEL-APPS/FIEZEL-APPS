...
import { AUTH_DDL, TEACHER_DDL, OAUTH_DDL } from '../workers/api/auth-schema.js';
...
const HEAD_OAUTH = `-- 0013_oauth_email.sql — IDENTITAS GOOGLE dan EMAIL.
--
-- DITURUNKAN OTOMATIS dari workers/api/auth-schema.js oleh
-- \`node tools/gen-auth-migrations.mjs\`. JANGAN sunting berkas ini langsung.
--
...
writeFileSync(
  new URL('../workers/api/migrations/0013_oauth_email.sql', import.meta.url),
  HEAD_OAUTH + '\n' + OAUTH_DDL.map(pretty).join('\n\n') + '\n'
);
console.log('gen-auth-migrations: 0011 + 0012 + 0013 ditulis ulang dari auth-schema.js');
