...
probe() { # $1=label  $2=pola nama assert yang harus FAIL
  ...
  node tests/analytics-privacy-test.js >/tmp/a5.probe.out 2>&1
  local verdict
...
echo "== hijau dasar =="
node tests/analytics-privacy-test.js >/dev/null 2>&1 && echo "HIJAU (exit 0)" || echo "DASAR SUDAH MERAH — hentikan"
...
node tests/analytics-privacy-test.js >/dev/null 2>&1 && echo "HIJAU kembali (exit 0)" || echo "MASIH MERAH — periksa pemulihan"
