 ...
 *
 * Pakai:
 *   node tools/dev/check-listening-similarity.js            laporan ringkas
 *   node tools/dev/check-listening-similarity.js --json     keluaran mesin
 *   node tools/dev/check-listening-similarity.js --threshold 0.25
 *
...
var BANK = path.join(__fzRoot, 'features', 'speaking-listening', 'listening-bank-v1.json');
...
function main() {
  ...
  if (args.json) {
    console.log(JSON.stringify({
      ...
      bank: path.relative(__fzRoot, BANK),
      items: items.length,
    ...
    console.log('Bank      : ' + path.relative(__fzRoot, BANK));
