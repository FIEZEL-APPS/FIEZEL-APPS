...
const SHOTS = [
  { name:'home-mobile',    view:'home',    w:390, h:844 },
  { name:'grammar-mobile', view:'grammar', w:390, h:844 },
  { name:'reading-mobile', view:'reading', w:390, h:844 },
  { name:'extra-mobile',   view:'vocab',   w:390, h:844 },
  { name:'home-desktop',   view:'home',    w:1440, h:900 },
];

for (const s of SHOTS){
  const ctx = await browser.newContext({ viewport:{width:s.w,height:s.h}, deviceScaleFactor:2, locale: LOCALE==='th'?'th-TH':'id-ID' });
  ...
  if (s.view!=='home'){ await page.evaluate(v=>window.go(v), s.view); await page.waitForTimeout(1800); }
