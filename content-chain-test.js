#!/usr/bin/env node
/**
 * GERBANG RANTAI KONTEN (content-chain-test.js)
 *
...
test('K7 · ownerDecisionRequired TIDAK PERNAH false, termasuk saat masukan mencoba menyetelnya', () => {
  ...
  const src = fs.readFileSync(path.join(__dirname, 'features', 'brain', 'fiezel-content-chain.js'), 'utf8');
...
test('gate ini terdaftar di CI', () => {
  ...
  assert.ok(workflow.includes('node content-chain-test.js'), 'gate yang tidak berjalan di CI bukan gate');
});
