...
test('PWA release coherence is m025-35 and caches Tutor v3 assets', () => {
    // Coherence, not a frozen N: Diagnostics and the shell must name the same build.
    const diagBuild = (fs.readFileSync('features/neural-voice/fiezel-diag-panel.js','utf8').match(/DIAG_BUILD\s*=\s*'([^']+)'/)||[])[1];
    assert.ok(diagBuild, 'DIAG_BUILD must be present');
    assert.ok(sw.includes("const SW_REV='" + diagBuild + "-"), 'SW_REV must carry the current build');
  ...
  assert.ok(sw.includes('./features/diagnostics/fiezel-diagnostic-register.js'), 'new main diagnostics assets stay cached');
    assert.ok(/var DIAG_BUILD = '[^']+'/.test(diag), 'DIAG_BUILD must be declared');
});
