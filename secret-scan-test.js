/**
 * secret-scan-test.js — GERBANG pemindai rahasia seluruh repo.
 *
 ...
 *
 * `edge-guard-test.js` sudah menjaga SATU berkas (`deploy/edge/api-index.php`).
 ...
 *       `edge-guard-test.js`) TANPA satu pun pengecualian berbasis nama berkas.
 *
 ...
 * penolakan secret di level render prompt (fiezel-prompt-library-test.js:39,
 * fiezel-self-refine-test.js:64). Menghukum keduanya berarti memaksa orang
...
const DETECTORS = [
  ...
  },
  {
    ...
    refine: (_match, groups) => {
      ...
      if (value.length < 12) return false;            // `a1b2c3d4` (fixture analytics-server-only-test.js:39)
 ...
 *     `edge-guard-test.js`, gerbang ini merah.
 *
 * Nilai yang dimaafkan di bawah adalah tiga fixture `edge-guard-test.js`
...
check('gerbang ini terdaftar di .github/workflows/quality.yml',
  /node secret-scan-test\.js/.test(workflow), 'quality.yml');
