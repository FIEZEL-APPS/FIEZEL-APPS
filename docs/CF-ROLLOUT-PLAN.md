...
>
> Gerbang yang menjaga berkas ini: `rollout-plan-test.js` (terdaftar di
...
| `FEATURE_AI` / `FEATURE_TTS` / `FEATURE_COACH` = `off`, `ANALYTICS_ENABLED = off` di Worker | `workers/api/wrangler.toml` `[vars]` |
...
| Mode `shadow` sudah ada di sakelar transport dan sudah diuji perilakunya (36 assert) | `app.js` blok `CF-TRANSPORT-BEGIN/END`, `cf-shadow-mode-test.js` |
...
| **Yang diukur** | (a) `Set-Cookie fz_id` lengkap (`HttpOnly`, `Secure`, `SameSite=Lax`, `Domain=fiezel.my.id`, `Max-Age`) — sudah dijamin `cf-live-contract-test.js` bila dijalankan; (b) rasio 200 pada `POST /api/auth/anon`; (c) laju penerbitan identitas anonim per jam; (d) rasio 401 pada `/api/user/me` untuk sesi yang seharusnya sah; (e) laporan "progres hilang" dari murid; (f) p99 CPU Worker. |
...
`localStorage`/state/DOM (`cf-shadow-mode-test.js`).
...
| **P3** | **Cache TTS belum diuji di runtime nyata.** Kunci cache v2 dan `HEAD R2` sebelum render diuji `tts-key-test.js` secara murni; **rasio cache-hit sungguhan belum pernah diukur** terhadap bucket `fiezel-audio` berisi 1.170 objek. Ambang batal R6 (< 70%) karena itu belum punya baseline. | `reports/exec-e5-ai-tts.md`, `tts-key-test.js` | **R6.** |
...
| **P5** | **Analytics Engine belum diaktifkan.** `ANALYTICS_ENABLED="off"` di `[vars]`, dan binding `AE` (`dataset = fiezel_events` di `workers/api`, `fiezel_ops` di `workers/owner`) belum terbukti aktif di akun. Membaca AE butuh SQL API + token akun (bukan binding), sehingga panel SYSTEM/AE **sengaja belum dibangun**. Konsekuensinya: `cache_hit/miss`, `quota_denied`, `circuit_open` — tepat metrik yang dipakai ambang R6 dan R7 — **belum bisa dibaca** dari mana pun. | `workers/api/wrangler.toml` `[vars]` + `[[analytics_engine_datasets]]`, `workers/owner/wrangler.toml` catatan AE | **R3, R6, R7.** Tanpa ini, ambangnya harus diukur manual lewat `wrangler tail` + dashboard Metrics, dan itu harus disepakati **sebelum** rilis, bukan saat insiden. |
