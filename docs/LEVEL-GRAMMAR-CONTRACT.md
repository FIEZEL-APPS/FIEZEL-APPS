...
```powershell
node .\tests/level-grammar-contract-test.js
node .\tests/grammar-curriculum-test.js
node .\tests/classroom-test.js
node .\tests/speaking-listening-test.js
```
...
```powershell
node --check .\app.js
node --check .\tests/level-grammar-contract-test.js
git diff --check
```
...
2. Jalankan `node tests/level-grammar-contract-test.js` sampai `PASS`.
