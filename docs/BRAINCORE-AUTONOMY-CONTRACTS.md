...
- **Dua gerbang anti-drift baru.** `tests/brain-page-wiring-test.js` W8: modul berotoritas `off`
  yang punya pemanggil di `app.js` = FAIL. `tests/brain-manifest-test.js`: otoritas sekarang
...
5. **Satu file test Node mandiri per modul** di root repo. Pola `tests/core-brain-v2-test.js`:
...
8. **Sebelum menyerahkan**, jalankan test milikmu DAN:
   `node tests/core-brain-v2-test.js && node tests/tutor-brain-v3-test.js && node tests/brain-manifest-test.js && node tests/brain-page-wiring-test.js && node tests/regression-test.js`
   ...
   dokumentasikan di komentar test (contoh presedennya ada di `tests/brain-manifest-test.js`).
...
|---|---|---|
| **selesai** | 1 | `app.js`, `index.html`, `sw.js`, `core-config.js`, `features/neural-voice/fiezel-diag-panel.js`, `features/brain/fiezel-brain-manifest.js`, `tests/brain-page-wiring-test.js`, `tests/brain-manifest-test.js` |
| **A-STAT** | 2 | `features/brain/fiezel-policy-verdict.js`, `tests/policy-verdict-test.js` |
| **A-NOF1** | 3 | `features/brain/fiezel-nof1.js`, `tests/nof1-test.js` |
| **A-CFG** | 4 | `features/brain/fiezel-brain-config.js`, `tests/brain-config-test.js`, `features/brain/fiezel-param-ledger.js`, `tests/param-ledger-test.js` |
| **A-TUNE** | 5 | `features/brain/fiezel-self-tune.js`, `tests/self-tune-test.js` |
| **A-GOV** | 6 | `fiezel-core-worker.js`, `tests/core-worker-contract-test.js`, `tools/run-content-gates.js`, `content-gate-runner-test.js`, `BRAINCORE-AUTONOMY-GOVERNANCE.md` |
| **A-WIRE** | integrasi | `app.js`, `index.html`, `sw.js`, `core-config.js`, `features/neural-voice/fiezel-diag-panel.js`, `features/brain/fiezel-brain-manifest.js`, `tests/brain-page-wiring-test.js`, `.github/workflows/quality.yml` |
...
TEST WAJIB (tests/policy-verdict-test.js):
...
SELESAI KALAU: node tests/policy-verdict-test.js PASS, dan
node tests/core-brain-v2-test.js && node tests/tutor-brain-v3-test.js && node tests/brain-manifest-test.js &&
node tests/brain-page-wiring-test.js && node tests/regression-test.js semuanya PASS.
...
SELESAI KALAU: node tests/nof1-test.js PASS + lima gate wajib di §1 butir 8 PASS.
...
KEPEMILIKAN EKSKLUSIF:
  ...
  tests/brain-config-test.js                    (edit)
  features/brain/fiezel-param-ledger.js   (baru)
  tests/param-ledger-test.js                    (baru)
...
1. FiezelBrainConfig.resolve(overrides, nowMs) sesuai §3. Wajib:
   ...
   - KONTRAK SINKRONISASI yang sudah ditegakkan tests/brain-config-test.js (default identik dengan
...
TEST WAJIB:
 - tests/brain-config-test.js: sinkronisasi default vs konstanta sumber tetap ditegakkan; resolve()
   menolak setiap kelas masukan korup dengan alasan; clamped[] benar-benar terisi saat dijepit.
 - tests/param-ledger-test.js: rantai utuh terverifikasi; tamper di tengah TERDETEKSI; rollback
   memulihkan nilai dan tercatat; determinisme hash; blok BUKTI-BISA-MERAH untuk keduanya.

SELESAI KALAU: node tests/brain-config-test.js && node tests/param-ledger-test.js PASS + lima gate wajib PASS.
...
TEST WAJIB (tests/self-tune-test.js):
...
SELESAI KALAU: node tests/self-tune-test.js PASS + lima gate wajib PASS.
...
KEPEMILIKAN EKSKLUSIF:
  fiezel-core-worker.js, tests/core-worker-contract-test.js
...
tests/core-worker-contract-test.js tetap PASS.
...
KEPEMILIKAN EKSKLUSIF:
  ...
  features/brain/fiezel-brain-manifest.js, tests/brain-page-wiring-test.js, .github/workflows/quality.yml
...
4. Perbarui authorityMap dengan JUJUR, dan naikkan bundleVersion. Ingat dua gerbang yang sudah
   ada: W8 di tests/brain-page-wiring-test.js menolak modul 'off' yang punya pemanggil, dan
   tests/brain-manifest-test.js sekarang MENURUNKAN otoritas dari pembacaan app.js dua arah. Kamu
...
SELESAI KALAU: seluruh daftar test di quality.yml PASS, termasuk tests/install-health-test.js dan
