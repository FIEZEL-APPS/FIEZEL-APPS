...
- **Bukti final:** karena perbaikan berjalan paralel, satu-satunya bukti yang sah untuk
  ...
  `node tests/regression-test.js`. Dokumen ini sengaja TIDAK mengklaim hasil PASS/FAIL apa pun
  pasca-perbaikan.
...
| 2 | **Deploy/konfigurasi Core Worker (Puter)** — bila hasil integrasi menyentuh `fiezel-core-worker.js` | Workflow `deploy-core-worker.yml`/`configure-core.yml` hanya `workflow_dispatch` dan digerbang aktor `fitrajft-ux` | `d-findings/D8-cloudflare-deploy.md` §2 |
