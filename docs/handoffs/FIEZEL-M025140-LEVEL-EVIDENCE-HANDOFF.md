...
- `tests/regression-test.js` mengunci teks `state.adaptiveReady=diagnosticEvidenceReady(state)`.
...
- `tests/level-grammar-contract-test.js` menandai `diagnosticReadinessMap` sebagai "cross-level pool".
...
- `tests/level-evidence-test.js`: **17/17 PASS**, menjalankan fungsi app.js yang asli di dalam `vm`.
