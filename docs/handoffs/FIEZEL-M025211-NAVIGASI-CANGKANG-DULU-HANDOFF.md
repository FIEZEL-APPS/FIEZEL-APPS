...
`sw-nav-budget-test.js` → **`tests/sw-nav-shell-first-test.js`** (8 → **15 assert**). Gerbang lama
...
1. `tests/pwa-startup-white-screen-recovery-test.js` — `indexOf('fetch(') < indexOf('caches.match(')`.
...
2. `tests/pwa-release-coherence-test.js` — "dokumen jaringan harus menang atas entri kosong".
...
mungkin: satu cabang navigasi. `tests/cf-config-killswitch-test.js` tidak disentuh dan tetap PASS.
