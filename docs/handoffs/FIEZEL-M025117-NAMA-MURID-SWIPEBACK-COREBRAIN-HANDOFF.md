...
komanya, plus `tests/library-integrity-test.js` sebagai gate baru. Saat `main` digabungkan ke
...
(`tests/observability-privacy-test.js` menjaga batas itu, dan rilis ini tidak melonggarkannya).
...
- **Baru:** `tests/core-brain-v2-test.js` (27 pemeriksaan; terdaftar di `quality.yml`). Ia tidak
...
- **Diperluas:** `tests/back-nav-test.js` (+11) mengunci keenam akar masalah; `tests/onboarding-test.js`
  (+8) mengunci langkah nama, termasuk bahwa ia wajib **dan** tidak mengurung. Fake DOM-nya
  kini melihat `<input>`, bukan hanya `<button>`.
- Seluruh gate `quality.yml` hijau, termasuk `tests/library-integrity-test.js` yang datang
