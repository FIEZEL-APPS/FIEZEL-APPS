...
- `tests/onboarding-test.js` (28 kasus, ditulis ulang total): goal ASLI bukan Travel/Work/Fun, level diberi label "perkiraan" eksplisit, 150 soal tidak disamarkan, tidak ada pemilih jadwal palsu, setiap langkah dan setiap slide carousel punya jalan keluar, navigasi maju-mundur carousel, tombol Lanjut nonaktif sebelum goal dipilih, gerbang notifikasi terverifikasi dipindah bukan dihapus.
- Semua gate tetangga hijau: `tests/pwa-cache-test.js`, `tests/pwa-release-coherence-test.js`, `tests/ui-structure-test.js`, `tests/a11y-test.js`, `validator.js`, `product-audit.js`, `tests/install-health-test.js`, `tests/alrs-behavior-test.js`, `tests/notification-reminder-test.js`, `tests/m02542-experience-test.js`, `tests/experience-integration-test.js`, `tests/regression-test.js`, `content-audit.js` (dua terakhir lambat, bukan gagal - dikonfirmasi keluar dengan kode 0 setelah dibiarkan selesai).
