...
`tests/contrast-test.js`, `tests/topbar-logo-contrast-test.js`, dan `tests/pastel-field-contrast-test.js`.
...
- **`tests/ui-structure-test.js`** memotong blok `@media` di kemunculan `'@media'` BERIKUTNYA,
  ...
  menghitung kurung.
- **`tests/splash-first-paint-test.js`** mencocokkan tag `<html …>` persis; sekarang mencocokkan
