...
| **A3** | **Nol notifikasi, di kedua sisi.** Pengundang tidak pernah tahu undangannya diterima. Penerima tidak pernah tahu ada undangan. Yang paling menyakitkan: `workers/api/social/notify-core.js` SUDAH ADA — 15 KB kebijakan permintaan teman + pusat notifikasi + push, lengkap dengan gerbangnya (`tests/social-notify-test.js`, 47 assert hijau) — tetapi **nol rute, nol tabel, nol pemanggil**. Kebijakan yang tidak pernah dieksekusi. | `grep -rn "notify-core" workers/api/` → hanya berkas itu sendiri |
...
| `tests/social-invite-link-test.js` | **baru** — 59 assert, terdaftar di `quality.yml` |
...
```
node tests/social-invite-link-test.js        59/59 assert PASS   (gerbang baru)
node tests/social-frontend-test.js           PASS (67 pemeriksaan)
node tests/social-api-contract-test.js       107/107 assert PASS
node tests/social-schema-contract-test.js    79/79 assert PASS
node tests/social-notify-test.js             47/47 assert PASS
node tests/boot-order-test.js                PASS
node tests/global-name-collision-test.js     22/22 PASS
...
node tests/id-golden-snapshot-test.js        HIJAU (sesudah regenerasi bagian 5)
node tests/th-coverage-test.js               143/143 PASS
node tests/contrast-test.js                  PASS
node tests/a11y-test.js                      PASS
node tests/ui-render-audit-test.js           semua invarian render hijau
...
awalnya memaku `color:#2A1D06` di atas `var(--gold)` dan `tests/contrast-test.js` menolaknya
...
- **Nyalakan lane `notify-core.js` di server.** Kebijakannya sudah ditulis dan sudah hijau;
  ...
  `tests/social-invite-link-test.js` yang mengunci kesamaan itu.
