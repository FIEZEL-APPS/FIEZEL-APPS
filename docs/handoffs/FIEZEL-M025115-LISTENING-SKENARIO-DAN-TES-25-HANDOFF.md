...
(baru), `listening-bank-v1.json`, `speaking-listening-config.js`, `tools/dev/check-listening-similarity.js`
(baru), `tools/dev/rebuild-speaking-listening-data.js`, `app.js`, `style.css`,
`features/onboarding/fiezel-onboarding.js`, `features/diagnostics/fiezel-diagnostic-targets.js`,
`tools/fiezel-guardians.mjs`, `tests/regression-test.js`, `tests/speaking-listening-test.js`,
`tests/onboarding-test.js`, `tests/diagnostic-scanner-test.js`, `.github/workflows/quality.yml`,
...
- `listening-lint.js` — aturan sama, tetapi melaporkan SEMUA pelanggaran sekaligus. Gerbang
  ...
  ratusan skenario.
- `tools/dev/check-listening-similarity.js` (root, terpasang di CI) — memeriksa bank yang SUDAH
