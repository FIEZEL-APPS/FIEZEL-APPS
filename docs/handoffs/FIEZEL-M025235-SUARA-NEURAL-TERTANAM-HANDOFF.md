...
- **JANGAN memasang kembali gerbang login.** `tests/voice-offline-fallback-test.js` kini menjaga
...
- **Ranjau gerbang bagi penyunting `sw.js` berikutnya**: `tests/neural-cache-isolation-test.js`
  melarang literal `'./vendor/` muncul di mana pun di sumber `sw.js` — termasuk di komentar.
  `tests/pwa-cache-test.js` dan `tests/sw-nav-shell-first-test.js` menuntut `skipWaiting` tepat satu kali
...
| Gerbang | Hasil |
|---|---|
| `tests/voice-offline-fallback-test.js` | 16 lulus (dari 14), termasuk 4 gerbang Background Fetch baru |
