...
| `tests/social-invite-link-test.js` | +9 assert (68/68) — lihat bagian 4 |
...
```
node tests/social-invite-link-test.js        68/68 assert PASS (naik dari 59)
node tests/social-frontend-test.js           PASS
node tests/social-api-contract-test.js       PASS
node tests/social-schema-contract-test.js    PASS
node tests/social-notify-test.js             PASS
node tests/boot-order-test.js                PASS
node tests/global-name-collision-test.js     PASS
node tests/install-health-test.js            PASS
node tests/pwa-release-coherence-test.js     PASS
node tests/coordination-guard-test.js        PASS
node tests/deploy-site-gate-test.js          PASS
node tests/gate-registry-test.js             PASS
node tests/contrast-test.js                  PASS
node tests/a11y-test.js                      PASS
node tests/locale-enum-test.js               PASS
...
Sembilan assert baru di `tests/social-invite-link-test.js` mengunci: banner bawaan Chrome ditahan
...
`tests/id-golden-snapshot-test.js`. Tiga lainnya (label tombol pendek, badan pesan, toast selesai)
tidak memenuhi ambang ≥2 kata penanda umum gerbang itu — bukan bug, ambang itu memang dirancang
presisi di atas recall (lihat komentar di kepala `tests/id-golden-snapshot-test.js`). Paritas id/th
penuh untuk keempatnya tetap terjaga lewat `tests/th-coverage-test.js` yang terpisah.
