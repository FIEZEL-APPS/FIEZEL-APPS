...
- Rantai penuh (murid → identitas → Braincore → bukti → D1 → API owner → Owner Dashboard)
  terpasang dan terbukti lewat gerbang `tests/braincore-learner-identity-test.js` (177 assert,
...
`fiezel-core`** (butir (g-g) `tests/owner-edge-guard-test.js` tetap hijau, 1117/1117). Data dibaca
...
`tests/braincore-learner-identity-test.js` — **177/177**, terdaftar di `quality.yml`. Ia
...
2. **Reset progres melewatkan dua kunci lane D** (`tests/reset-side-state-test.js` merah).
...
5. **`coordination/BUILD-VERSION.json` tertinggal di m025-229** sementara tiga penanda
   build sudah m025-230 (`tests/coordination-guard-test.js` merah). Sumber tunggal nomor build
...
bagian dari cacat #2: lexer `tests/id-golden-snapshot-test.js` membaca blok komentar di sekitar
...
1. `tests/config-consistency-test.js` — hitungan flag 7→8 dan kill switch 5→6. Gerbang itu memang
   dirancang supaya penambahan flag tidak bisa lewat tanpa dilihat orang.
2. `tests/brain-sync-failclosed-test.js` — jangkar racunnya (`brainSync:false}`) sudah tidak ada
