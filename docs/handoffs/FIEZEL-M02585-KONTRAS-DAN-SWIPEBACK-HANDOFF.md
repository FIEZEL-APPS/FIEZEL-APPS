...
5. **Tes regresi.** Tambahkan `tests/contrast-test.js`: baca `style.css` + `tutor-v3.css`,
...
Playwright); pakai `tests/contrast-test.js` statis (langkah 5) sebagai gerbang CI.
...
Suite lokal 89/89 lulus, ditambah regresi `tests/splash-first-paint-test.js` (21 pemeriksaan).
...
Gerbang barunya `tests/contrast-test.js`: 46 pasang x 3 keadaan = 138 rasio WCAG, tanpa browser,
