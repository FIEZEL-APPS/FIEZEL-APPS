...
**Dijaga:** `tests/ux-flags-test.js` T6b — MERAH kalau ada yang memagari unduhan latar itu lagi
...
**Dijaga:** `tests/audio-locale-guard-test.js` — SETIAP sebutan `speechSynthesis` /
...
```
node tests/audio-locale-guard-test.js     # TTS peramban mati total, audio locale-independent
node tests/ux-flags-test.js               # T6b: unduhan latar tidak berpagar
node tests/neural-cache-isolation-test.js # cache suara tidak tercampur antar akun
```
