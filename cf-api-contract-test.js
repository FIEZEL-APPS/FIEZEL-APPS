/**
 * cf-api-contract-test.js — GERBANG kontrak Worker API Cloudflare (`workers/api/`).
 *
...
const API_DIR = path.join(__dirname, 'workers', 'api');
const results = [];
let failures = 0;
...
(async () => {
  ...
  /* ---------- 8. /api/config: kill switch server-side ---------------------- */
  {
    ...
    assert(!('workerUrl' in flags), "flags TIDAK memakai field workerUrl (remote-push-test.js:6 mengunci regex *.puter.work)");
  ...
  /* ---------- 13. wrangler.toml tetap PLAN GRATIS -------------------------- */
  {
    ...
    const audioToml = fs.readFileSync(path.join(__dirname, 'workers', 'wrangler.toml'), 'utf8');
