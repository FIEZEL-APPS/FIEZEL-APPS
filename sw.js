...
self.addEventListener('fetch',e=>{
  ...
  if(new URL(e.request.url).pathname.toLowerCase().endsWith('/version.json')){
    e.respondWith(fetch(e.request).then(r=>r&&r.ok?r:caches.match(e.request)).catch(()=>caches.match(e.request)));
    return;
  }
  e.respondWith(caches.match(e.request).then(c=>{
    if(c)return e.request.mode==='navigate'?withIsolationHeaders(c):c;
    return fetch(e.request).then(r=>{
      if(r&&r.ok&&!isNeuralAsset(e.request)){
        const copy=r.clone();
        caches.open(CACHE).then(cache=>cache.put(e.request,copy));
      }
      return e.request.mode==='navigate'?withIsolationHeaders(r):r;
    }).catch(error=>{
      if(e.request.mode==='navigate')return caches.match('./index.html').then(withIsolationHeaders);
      throw error;
    });
  }));
});
